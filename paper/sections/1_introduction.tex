...
\begin{enumerate}
    ...
    \item \textbf{Empirical evaluation on a calibrated corridor.} On a stochastic microsimulation under a unified 300-episode training and held-out evaluation protocol, TransitDuet matches or marginally outperforms every baseline on every metric we report: composite cost $1.444$ vs $1.504$ for the next-best baseline (CMA-ES, $-4.0\%$) and $1.544$ for Fixed ($-6.5\%$), with seed variance $1.7$--$2.5\times$ smaller than the search baselines. The wait/CV margins ($-0.7\%$ / $-1.7\%$ vs GA/CMA-ES) fall within seed noise; we report all $n{=}3$-seed ranges and avoid pairwise significance claims.
\end{enumerate}

\paragraph{Results preview.}
We evaluate on a high-frequency bus corridor with 22 stations, 264 daily trips, demand stochasticity $\sigma_{\mathrm{demand}} = 0.15$, and elastic fleet $\Nfleet \in [8, 16]$ sampled per episode. Under a unified 300-episode training and held-out evaluation protocol, TransitDuet matches or marginally outperforms every baseline on wait, CV, overshoot, and composite cost simultaneously --- $5.5$/$6.4$/$10.4$/$6.5\%$ improvements over the strongest hand-tuned Fixed timetable, with smaller margins on the search baselines that fall within seed noise --- while attaining the lowest composite-cost variance of any method and degrading only $25\%$ on wait when route stochasticity is doubled beyond the training distribution.
