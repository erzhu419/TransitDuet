...
\begin{enumerate}
    ...
    \item \textbf{Empirical evaluation on a calibrated corridor.} On a stochastic microsimulation under a unified 300-episode training and held-out evaluation protocol, TransitDuet matches or marginally outperforms every baseline on every metric we report: composite cost $1.444$ vs $1.488$ for the next-best baseline (CMA-ES, $-3.0\%$) and $1.544$ for Fixed ($-6.5\%$). GA in our 300-episode from-scratch setup exhibits a 1-in-3 cold-start failure mode (one of three seeds converges to high CV) that pushes its mean composite to $1.800$ with std $0.458$, illustrating the unreliability of population-based search at this budget.
\end{enumerate}

\paragraph{Results preview.}
We evaluate on a high-frequency bus corridor with 22 stations, 264 daily trips, demand stochasticity $\sigma_{\mathrm{demand}} = 0.15$, and elastic fleet $\Nfleet \in [8, 16]$ sampled per episode. Under a unified 300-episode training and held-out evaluation protocol, TransitDuet matches the strongest external baseline (CMA-ES) within seed noise on wait while improving CV by $7.5\%$, overshoot by $4.9\%$, and composite cost by $3.0\%$; against the Fixed timetable the gap on every metric widens to $5$--$10\%$. GA in this protocol shows a 1-in-3 cold-start failure mode that brings its composite to $1.800 \pm 0.458$. TransitDuet degrades only $25\%$ on wait when route stochasticity is doubled beyond the training distribution.
