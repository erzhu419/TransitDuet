...
\begin{enumerate}
    \item Goal-conditioned cross-level coupling. We adopt a HIRO-style goal-conditioned coupling~\citep{nachum2018data}: the upper outputs a per-dispatch target-headway shift, and the lower's Lagrangian cost is computed against the resulting goal headway, with no upper-advantage injection into the lower's transitions. We compare this end-to-end against (i) a literal cross-advantage augmentation following HAAR~\citep{li2019hierarchical} with a PIPER-style reachability gate~\citep{singh2024piper}, and (ii) a three-channel coupling that uses $\delta_t$ as a launch-time perturbation with hindsight reward credit; the goal-conditioned design has the best mean composite cost ($1.418$ vs $1.496$ vs $1.521$) and the lowest seed variance ($\geq 5\times$ tighter than the alternatives) on the same evaluation protocol.
    \item Joint target-headway--holding formulation. We cast adaptive bus target-headway planning and holding control as a bi-level Markov decision process with heterogeneous state-action spaces and asynchronous transitions; the upper level sets per-dispatch target headways atop a fixed nominal timetable rather than redesigning frequencies offline, providing a formal problem structure absent from prior bus RL work.
    \item Adaptive constraint handling via $\thetavec$-OGD and Lagrangian relaxation. We adaptively penalise fleet overshoot at the upper level via measurement-driven OGD on the reward weights and headway deviation at the lower level via a dual variable. Both are soft constraints in the sense that they shape the reward rather than project the policy; the OGD scheme removes the need to hand-tune an overshoot coefficient (as a reward-shaping baseline would require) but does not provide a worst-case feasibility guarantee.
    \item Target-policy-corrected lower SAC (TPC). We identify a covariate-shift failure mode of joint bi-level RL on tightly coupled control: the lower policy is trained alongside an exploring upper policy, biasing the lower's training distribution away from deployment. We address this via importance-weighted lower SAC against an EMA ``deployment'' upper policy, with validation-based checkpoint selection. Under the goal-conditioned coupling we ultimately adopt, the upper modifies the lower's target rather than the distribution of states it sees, so TPC's in-distribution effect is small (\cref{sec:ablation}); its main practical value is on the launch-time-perturbation coupling we tested as an alternative.
    \item Empirical evaluation on a calibrated corridor. On a stochastic microsimulation under a near-unified, approximately budget-matched protocol (TransitDuet 300 episodes per seed; search baselines 320 episodes = 20 lower warmup + 300 search-evaluation), TransitDuet matches or improves on every baseline on its strongest metrics: composite cost $1.418$ vs $1.488$ for the next-best baseline (CMA-ES, $-4.7\%$) and $1.544$ for Fixed ($-8.2\%$). GA in our 300-episode from-scratch setup exhibits a 1-in-3 cold-start failure mode (one of three seeds converges to high CV) that pushes its mean composite to $1.800$ with std $0.458$, illustrating the unreliability of population-based search at this budget.
\end{enumerate}

\paragraph{Results preview.}
We evaluate on a high-frequency bus corridor with 22 stations, 264 daily trips, demand stochasticity $\sigma_{\mathrm{demand}} = 0.15$, and elastic fleet $\Nfleet \in [8, 16]$ sampled per episode. Under a near-unified, approximately budget-matched protocol (300 vs 320 simulator episodes per seed for TransitDuet vs the search baselines), TransitDuet improves on the strongest external baseline (CMA-ES) by $2.5\%$ on wait, $12.1\%$ on CV, and $4.7\%$ on composite cost; the overshoot margin over CMA-ES ($1.60$ vs $1.63$) is within seed noise and we do not claim a substantive overshoot advantage. Against the Fixed timetable the gap widens to $7$--$11\%$ on every metric. GA in this protocol shows a 1-in-3 cold-start failure mode that brings its composite to $1.800 \pm 0.458$. TransitDuet degrades only $27\%$ on wait when route stochasticity is doubled beyond the training distribution.
