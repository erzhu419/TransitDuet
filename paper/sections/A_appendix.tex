...
\begin{enumerate}
  \item Holding --- the bus is held at a station for the duration specified by the lower-level policy (0--60\,s).
...
We resolve this through a trip-lifecycle alignment. Each upper action $a_{\mathrm{U},i}$ at dispatch $t_i^{\mathrm{U}}$ initiates a trip $\tau_i$ launched at the baseline time $t_{\mathrm{base},i}$ with a dispatch-specific target headway $\htarget + \delta_{t,i}$. The realised inter-dispatch gap is the actual elapsed time between consecutive bus departures as carried out by the lower controller (which may hold individual buses to track its target):
\begin{equation}
