...
\noindent External baselines:
\begin{itemize}[leftmargin=*]
    \item Fixed: static timetable with $H = 360$\,s throughout the day; only the lower-level holding controller is trained. A strong hand-tuned baseline.
    \item GA: genetic-algorithm search over $[\Hpeak, \Hoff, \Htrans]$ (pop\,size\,15, $\sigma_{\text{mut}}=0.15$).
    \item CMA-ES: covariance-matrix adaptation evolution strategy (pop\,size\,10, $\sigma_0 = 0.3$).
\end{itemize}
...
\noindent Component ablations (all start from TransitDuet full):
\begin{itemize}[leftmargin=*]
    \item --HoldFB: remove holding-feedback coupling from the upper state (state dims $5$--$7$ zeroed).
    \item --CS-BAPR: disable the Bayesian changepoint belief tracker; upper $\alpha$ stays at nominal value.
    \item --Hindsight: disable the per-dispatch gap-deviation credit; upper uses only episode-level system reward.
    \item --MORL: replace belief-weighted $\thetavec$-OGD scalarization with fixed weights $[0.5, 0.25, 0.25]$.
    \item --Elastic: fix $\Nfleet = 12$ during training (policy does not see elastic budget).
    \item --DemandNoise: deterministic demand (remove the per-hour multiplier and peak-time shift).
\end{itemize}
...
\begin{itemize}
    \item Average passenger wait time (min): time between a passenger's arrival at a station and boarding.
    \item Headway coefficient of variation (CV): $\sigma_h / \mu_h$ across inter-dispatch gaps within an episode; captures regularity.
    \item Fleet overshoot: $\max(0, \text{peak fleet} - \Nfleet)$; non-zero when the policy exceeds the sampled budget.
    \item Composite cost: $\mathrm{wait}/10 + \mathrm{overshoot}^2 / \Nfleet + \mathrm{CV}$, the upper-level training objective. Lower is better on all four.
\end{itemize}
...
\midrule
\multicolumn{5}{@{}p{\textwidth}@{}}{Rule-lower baselines (proportional headway-keeping rule; 1 seed, $n{=}20$ held-out eval)} \\
...
\midrule
\multicolumn{5}{@{}p{\textwidth}@{}}{Per-candidate-retrain GA (each candidate fine-tunes a fresh-from-warm-start lower; 1 seed)} \\
Per-cand-GA          & 16.82\,$\pm$\,5.95 & 0.567\,$\pm$\,0.096 & 2.15\,$\pm$\,1.24 & 2.917\,$\pm$\,0.911 \\
\midrule
TransitDuet (HIRO, Ours)  & 5.11\,$\pm$\,0.17 & 0.434\,$\pm$\,0.008 & 1.60\,$\pm$\,0.04 & 1.418\,$\pm$\,0.016 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Variant & Wait (min) & CV & Composite & $\Delta$Composite \\
\midrule
TransitDuet (full)          & 5.11\,$\pm$\,0.17 & 0.434\,$\pm$\,0.008 & 1.418\,$\pm$\,0.016 & \;--- \\
\midrule
...
The asymmetry has a clean explanation: under the goal channel, the upper changes the lower's constraint target but not the distribution of states the lower observes, so the lower's covariate shift from upper exploration is much milder, and the importance-sampling correction in TPC has less work to do.
...
capacity-limited ($\Nfleet \in [8, 11]$) where the policy saturates the cap (overshoot $\equiv 3$) and trades fleet violation for wait reduction --- absolute-wait minimum is $\Nfleet = 10$ ($4.55$\,min);
and feasible-overshoot ($\Nfleet \in [12, 16]$) where the policy reduces overshoot toward $0$ (zero by $\Nfleet \geq 15$) at a small wait cost.
