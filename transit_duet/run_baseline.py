...
Key changes vs runner.py for speed:
  - training_freq = 50 (train every 50 transitions, not 5)
  - batch_size = 512 (lighter per update)
  - Episode-end batch training (10 updates after each episode)
  - No upper policy in this baseline (fixed timetable, target_headway=360s)

Usage:
    cd transit_duet/
    python -u run_baseline.py --episodes 80 [--seed 42] [--gpu]
"""
...
def run_baseline(args):
    ...
    # ---- Lower policy (DSAC-Lagrangian) ----
    trainer = DSACLagrangianTrainer(
        state_dim=state_dim,
        action_dim=1,
        hidden_dim=64,         # bigger than default 32
        action_range=60.0,
        cost_limit=0.15,
    ...
    batch_size = 512
    updates_per_episode = 20   # batch updates at episode end
    warmup_episodes = 2        # collect data before training

    # ---- Logging ----
    log_dir = SCRIPT_DIR / 'logs' / f'baseline_seed{args.seed}'
    os.makedirs(log_dir, exist_ok=True)
    history = defaultdict(list)

    print(f"TransitDuet Baseline: {args.episodes} episodes, seed={args.seed}")
    print(f"  state_dim={state_dim}, device={device}, hidden=64, lr=3e-4")
    ...
    for ep in range(args.episodes):
        ...
        cost_q_mean = train_metrics.get('cost_q_mean', 0)

        print(f"[Ep {ep:3d}] R={ep_reward:8.1f} | avg_r={avg_reward:6.2f} | "
              f"steps={ep_steps:4d} | "
              f"wait={z[0]:5.1f}m | fleet={z[1]:2.0f} | bunch={z[2]:.3f} | "
              f"λ={trainer.lambda_param:.3f} α={trainer.alpha:.3f} | "
              f"Q={q_mean:.1f} Qc={cost_q_mean:.3f} | "
    ...
    print(f"Final: avg_wait={history['avg_wait'][-1]:.1f}min, "
          f"bunching={history['bunching_rate'][-1]:.3f}, "
