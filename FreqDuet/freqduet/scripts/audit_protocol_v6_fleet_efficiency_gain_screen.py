#!/usr/bin/env python3
"""Audit the preregistered V16 fleet-conditioned efficiency-gain screen."""
...
PRESSURE_EXPONENTS = (("1", 1.0), ("2", 2.0))
PRESSURE_START = 0.9
PRESSURE_FULL = 1.0
CANDIDATE_SPECS = [
    (
        "F_freqduet_protocol_v6_w2adfleetgain_l001_e25_r00025_"
        f"w{weight_name}_b{penalty_name}_p{exponent_name}_hiro",
        weight,
        penalty,
        exponent,
    )
    for exponent_name, exponent in PRESSURE_EXPONENTS
...
TRAIN_SEEDS = [20013, 20031, 20053, 20077]
EVAL_SEEDS = [53017, 53041, 53059, 53083]
PRIORITY = [
    "F_freqduet_protocol_v6_w2adfleetgain_l001_e25_r00025_"
...
def evaluate_fleet_efficiency_gain_screen(
    aggregate_dir: Path,
) -> dict[str, object]:
    ...
    if missing:
        raise FileNotFoundError(f"missing V16 aggregate artifacts: {missing}")
    ...
    if not all(strict_checks.values()):
        raise ValueError(f"V16 strict checks failed: {strict_checks}")
    ...
    if missing_columns:
        raise ValueError(f"V16 metrics are missing: {missing_columns}")
    ...
    candidate_results = []
    for candidate, weight, penalty, pressure_exponent in CANDIDATE_SPECS:
        ...
        if len(rows) != expected_pairs or len(anchor_pair) != 1:
            raise ValueError(f"incomplete V16 candidate: {candidate}")
        ...
        efficiency_penalty = numeric(
            "lower_regularity_policy_action_efficiency_penalty")
        pressure_start = numeric(
            "lower_regularity_policy_fleet_pressure_start")
        pressure_full = numeric(
            "lower_regularity_policy_fleet_pressure_full")
        observed_pressure_exponent = numeric(
            "lower_regularity_policy_fleet_pressure_exponent")
        ...
        mechanism_checks = {
            ...
            "fleet_efficiency_gain_mode_locked": bool(
                (enabled == 1.0).all()
                and (policy_mode
        ...
        outcome_checks = {
            ...
            "action_does_not_increase_vs_v14": candidate_action < v14_action,
        ...
        candidate_results.append({
            ...
            "expected_action_efficiency_penalty": penalty,
            "expected_fleet_pressure_exponent": pressure_exponent,
    ...
    return {
        "gate_version": "freqduet-v16-fleet-efficiency-gain-screen-v1",
        "status": (
...
def main() -> None:
    ...
    result = evaluate_fleet_efficiency_gain_screen(args.aggregate_dir)
    out = args.out or args.aggregate_dir / "fleet_efficiency_gain_screen.json"
