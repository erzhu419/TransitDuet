...
### Engineering-v33 timescale-aligned stability preregistration (2026-09-11)

The completed manuscript audit leaves one scientific blocker: V8 confirms the
compact two-sided regularity mechanism at 40 episodes, but its headway-CV
advantage decays during the unrestricted V9 200-episode run. V9 trajectory
evidence places the decay after approximately episode 100 while the causal
regularity reward remains active. V10--V27 reject stronger soft constraints,
dual objectives, categorical projections, forecast calibration, and
observational multi-step value targets; V28--V32 reject the available
low-capacity terminal prefix selectors. V33 therefore tests training timescale,
...
`0.02` with crossed-bootstrap CI fully below zero; restricted-journey CI high
must be at most `+0.15 min`; at least 75% of training-seed mean CV deltas must
be negative; matched-context, holding, denied-dispatch, causal-evidence, and
zero-execution-adjustment gates must all pass. The first passing candidate in
