...
from scripts.audit_protocol_v6_fleet_efficiency_gain_screen import (
    CANDIDATE_SPECS,
    CONFIGS,
    EVAL_SEEDS,
    PRIORITY,
    ...
    V14_ANCHOR,
    V15_NEAR,
    evaluate_fleet_efficiency_gain_screen,
)


class ProtocolV6FleetEfficiencyGainScreenTest(unittest.TestCase):
    def _artifacts(self, root: Path, *, active_pressure: bool = True):
        ...
        spec_by_name = {
            name: (weight, penalty, exponent)
            for name, weight, penalty, exponent in CANDIDATE_SPECS
        }
        rows = []
        for config in CONFIGS:
            ...
            if candidate:
                journey, cv, holding, denied, action = (
                    20.10, 0.195, 130.0, 130.0, 12.5)
                weight, penalty, exponent = spec_by_name[config]
                pressure = 0.8 if active_pressure else 1.0
                efficiency = 1.0 / (1.0 + 0.5 * penalty * pressure)
            else:
                journey, cv, holding, denied, action = values[config]
                weight, penalty, exponent = 0.0, 0.0, 1.0
                pressure, efficiency = 0.0, 1.0
        ...
        pd.DataFrame([{
            ...
            "delta_headway_cv_mean": -0.035,
            "delta_headway_cv_ci_low": -0.045,
            "delta_headway_cv_ci_high": -0.02,
        } for candidate, _, _, _ in CANDIDATE_SPECS]).to_csv(
            root / "frozen_paired_deltas.csv", index=False)

    def test_selects_the_first_passing_fleet_candidate(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._artifacts(root)
            result = evaluate_fleet_efficiency_gain_screen(root)
    ...
    def test_saturated_pressure_signal_cannot_pass(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._artifacts(root, active_pressure=False)
            result = evaluate_fleet_efficiency_gain_screen(root)

        self.assertEqual(result["status"], "no_pass")
        self.assertTrue(all(
            not item["mechanism_checks"][
                "fleet_signal_active_in_every_rollout"]
    ...
    def test_wrong_seed_grid_fails_before_selection(self):
        with TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "strict checks"):
                evaluate_fleet_efficiency_gain_screen(root)
