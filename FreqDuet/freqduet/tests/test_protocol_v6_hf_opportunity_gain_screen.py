...
from scripts.audit_protocol_v6_hf_opportunity_gain_screen import (
    CANDIDATE_SPECS,
    CONFIGS,
    EVAL_SEEDS,
    PRIORITY,
    ...
    V17_CV,
    V17_JOURNEY,
    evaluate_hf_opportunity_gain_screen,
)


class ProtocolV6HFOpportunityGainScreenTest(unittest.TestCase):
    def _artifacts(
        self,
        root: Path,
        *,
        ...
        values = {
            ...
            V15_NEAR: (20.18, 0.205, 135.0, 135.0, 12.7),
            V16_NEAR: (20.14, 0.200, 132.0, 132.0, 12.6),
            V17_CV: (20.16, 0.195, 134.0, 134.0, 12.6),
            V17_JOURNEY: (20.10, 0.235, 128.0, 128.0, 12.4),
        }
        spec_by_name = {
            name: (weight, penalty)
        ...
        for config in CONFIGS:
            ...
            if candidate:
                journey, cv, holding, denied, action = (
                    20.10, 0.190, 130.0, 130.0, 12.5)
                weight, penalty = spec_by_name[config]
                hf_energy = 0.04
                hf_pressure = 0.5
                hf_gate = 1.0 / (1.0 + penalty * hf_pressure)
            else:
                journey, cv, holding, denied, action = values[config]
                weight, penalty = 0.0, 0.0
                hf_energy, hf_pressure, hf_gate = 0.0, 0.0, 1.0
            for train_seed in TRAIN_SEEDS:
                for eval_seed in EVAL_SEEDS:
                    ...
                    rows.append({
                        ...
                        "lower_regularity_policy_action_efficiency_penalty": 0.0,
                        "lower_regularity_policy_hf_energy_scale": (
                            0.05 if candidate and wrong_hf_scale else
                            (0.04 if candidate else 1.0)),
                        "lower_regularity_policy_hf_energy_exponent": 1.0,
                        ...
                        "lower_regularity_policy_action_efficiency_gate_mean": (
                            hf_gate),
                        "lower_regularity_policy_hf_energy_mean": hf_energy,
                        "lower_regularity_policy_hf_energy_pressure_mean": (
                            hf_pressure),
                        "lower_regularity_policy_actor_hf_energy_mean": (
                            hf_energy if contaminated_actor_metric else 0.0),
                        "lower_regularity_policy_actor_hf_energy_pressure_mean": 0.0,
                        "lower_regularity_policy_actor_action_efficiency_gate_mean": 0.0,
                    })
        ...
        pd.DataFrame([{
            ...
            "delta_headway_cv_ci_high": -0.020,
        } for candidate, _, _ in CANDIDATE_SPECS]).to_csv(
            root / "frozen_paired_deltas.csv", index=False)

    def test_selects_the_first_passing_candidate(self):
    ...
    def test_wrong_hf_scale_cannot_pass(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._artifacts(root, wrong_hf_scale=True)
            result = evaluate_hf_opportunity_gain_screen(root)
    ...
    def test_wrong_seed_grid_fails_before_selection(self):
        with TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "strict checks"):
                evaluate_hf_opportunity_gain_screen(root)
