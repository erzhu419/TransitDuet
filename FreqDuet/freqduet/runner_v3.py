...
class DiagnosticLog:
    ...
    HEADER = [
        ...
        'lower_regularity_projection_actor_forward_kl',
        'lower_regularity_projection_actor_distillation_steps',
        'lower_regularity_projection_actor_post_reverse_kl',
        ...
        'lower_regularity_projection_actor_post_forward_kl_episode_max',
...
class TransitDuetV2Runner:
    ...
    def __init__(self, config, device='cpu'):
        ...
        if bool(regularity_policy_cfg.get('enable', False)):
            ...
            if evidence_mode != 'compact_causal_target_v7':
                raise ValueError(
                    'causal regularity policy requires '
                    'evidence_mode=compact_causal_target_v7')
            if self.lower_action_bins is None or (
                    self.lower_action_bins_gate_enabled):
                raise ValueError(
                    'causal regularity policy requires fixed action_bins')
            if self.lower_state_encoder is None:
                raise ValueError(
                    'causal regularity policy requires the physical state encoder')
            required_features = {
                'regularity_hold_target_norm',
                'regularity_hold_target_valid',
            }
            missing_features = required_features.difference(
                self.env.lower_context_features)
            if missing_features:
                raise ValueError(
                    'causal regularity policy is missing compact causal features: '
                    + ', '.join(sorted(missing_features)))
            base_state_dim = int(self.env._base_state_dim)
            regularity_policy_cfg.update({
                'target_feature_index': (
                    base_state_dim + self.env.lower_context_features.index(
                        'regularity_hold_target_norm')),
                'valid_feature_index': (
                    base_state_dim + self.env.lower_context_features.index(
                        'regularity_hold_target_valid')),
                # PhysicalLowerStateEncoder stores target/headway_norm in slot 0.
                'target_headway_feature_index': 0,
                'action_target_scale_s': float(
                    self.env.lower_causal_holding_action_scale_s),
            ...
            if regularity_mode in {
                ...
                if regularity_mode in {
                        'analytic_two_sided_fleet_efficiency_gain_regret_dual_v5',
                        'analytic_two_sided_target_preserving_gain_regret_dual_v6'}:
                    if 'fleet_utilization' not in self.env.lower_context_features:
                        raise ValueError(
                            'fleet efficiency gain requires the causal '
                            'fleet_utilization lower-context feature')
                    capacity_gain_cfg[
            ...
            if regularity_mode in {
                ...
                if (not self.env.frequency_lower_enabled
                        or lower_frequency_mode not in {
                            'high', 'hf', 'split'}):
                    raise ValueError(
                        'HF gain floor requires high-mode causal lower '
                        'frequency features')
                gain_floor_cfg = copy.deepcopy(
                    regularity_policy_cfg.get(
                ...
                gain_floor_cfg['hf_energy_feature_index'] = (
                    ...
                    + 2)
                regularity_policy_cfg[
                    'regularity_gain_floor'] = gain_floor_cfg
        ...
        self._ep_lower_regularity_policy_action_efficiency_gates = []
        ...
        self._ep_lower_regularity_gain_floor_required_fractions = []
        self._ep_lower_regularity_gain_floor_hf_pressures = []
        self._ep_lower_regularity_gain_floor_expected_gain_fractions = []
    ...
    def _lower_policy_action(self, obs, last_action=0.0, deterministic=False):
                ...
                if self.lower_trainer.regularity_policy_enabled:
                    ...
                    if bool(valid.item() >= 0.5):
                        self._ep_lower_regularity_policy_valid_entropies.append(
                            entropy)
                if self.lower_trainer.regularity_gain_floor_enabled:
                    (
                        floor_valid,
                        _,
                        floor_hf_pressure,
                        floor_required,
                        floor_gain_terms,
                        floor_shortfall_terms,
                        floor_eligible,
                    ) = self.lower_trainer._regularity_gain_floor_action_terms(
                        state_tensor.unsqueeze(0))
                    if bool(floor_valid.item() >= 0.5):
                        eligible = float(floor_eligible.item())
                        expected_shortfall = float((
                            probs * floor_shortfall_terms
                        ).sum(dim=-1).item())
                        self._ep_lower_regularity_gain_floor_eligible.append(
                            eligible)
                        ...
                        if eligible >= 0.5:
                            ...
                            self._ep_lower_regularity_gain_floor_required_fractions.append(
                                float(floor_required.item()))
                            self._ep_lower_regularity_gain_floor_hf_pressures.append(
                                float(floor_hf_pressure.item()))
                            self._ep_lower_regularity_gain_floor_expected_gain_fractions.append(
                                expected_gain)
                            floor_gain_fractions = floor_gain_terms.reshape(-1)
        ...
        if floor_gain_fractions is not None or floor_shortfalls is not None:
            bins = self.lower_trainer.discrete_actions.reshape(-1)
            selected_index = int(torch.argmin(torch.abs(
                bins - self._lower_action_scalar(action))).item())
            if floor_gain_fractions is not None:
                self._ep_lower_regularity_gain_floor_selected_gain_fractions.append(
                    float(floor_gain_fractions[selected_index].item()))
    ...
    def _record_causal_regularity_policy_execution(
        ...
        if not compact_features.issubset(self.env.lower_context_features):
            return
        contract = self.lower_trainer.regularity_policy_contract
        action_cap_s = float(
            contract.get(
                'action_target_scale_s',
                self.env.lower_causal_holding_action_scale_s))
        cost_cap = float(contract.get('cost_cap', 0.25))
        target_action = None
        target_headway = None
        if (context is not None and context.evidence_valid
                and context.follower_evidence_valid):
            target_action = causal_two_sided_holding_target_s(
                forward_headway_s=context.forward_headway_s,
                follower_departure_gap_s=context.follower_departure_gap_s,
                action_cap_s=action_cap_s,
            )
            target_headway = context.target_headway_s
        valid = target_action is not None and target_headway is not None
        self._ep_lower_regularity_policy_evidence_valid.append(
            ...
            if hf_feature_index is not None:
                ...
                action_efficiency_gate = holding_action_efficiency_gate(
                    ...
                    penalty=float(getattr(
                        self.lower_trainer,
                        'regularity_capacity_action_efficiency_penalty',
                        0.0)),
                )
            else:
                fleet_feature_index = int(fleet_feature_index)
                if not 0 <= fleet_feature_index < encoded_state.size:
                    raise RuntimeError(
                        'fleet-utilization regularity feature index is out of '
                        'range during action execution')
                fleet_utilization = float(np.clip(
                    encoded_state[fleet_feature_index], 0.0, 1.0))
                fleet_pressure = fleet_utilization_pressure(
                    utilization=fleet_utilization,
                    pressure_start=float(
                        self.lower_trainer
                        .regularity_capacity_fleet_pressure_start),
                    pressure_full=float(
                        self.lower_trainer
                        .regularity_capacity_fleet_pressure_full),
                    exponent=float(
                        self.lower_trainer
                        .regularity_capacity_fleet_pressure_exponent),
                )
                if (self.lower_trainer.regularity_capacity_gain_mode
                    ...
                    action_efficiency_gate = (
                        holding_target_preserving_fleet_efficiency_gate(
                            ...
                            penalty=float(
                                self.lower_trainer
                                .regularity_capacity_opportunity_cost_penalty),
                            fleet_pressure=fleet_pressure,
                            target_pressure_exponent=float(
                                self.lower_trainer
                                .regularity_capacity_target_pressure_exponent),
                        ))
                    self._ep_lower_regularity_policy_target_pressures.append(
                        target_pressure)
                else:
                    action_efficiency_gate = holding_fleet_efficiency_gate(
                        action_s=float(action_s),
                        action_scale_s=action_cap_s,
                        penalty=float(
                            self.lower_trainer
                ...
                self._ep_lower_regularity_policy_fleet_utilizations.append(
                    fleet_utilization)
                self._ep_lower_regularity_policy_fleet_pressures.append(
                    fleet_pressure)
            capacity_gain = capacity_gate * action_efficiency_gate * max(
                float(zero_hold_action_cost) - float(action_cost), 0.0)
            self._ep_lower_regularity_policy_capacity_gates.append(
                capacity_gate)
            self._ep_lower_regularity_policy_action_efficiency_gates.append(
                action_efficiency_gate)
        ...
        self._ep_lower_regularity_policy_abs_errors.append(
            abs(float(action_s) - float(target_action)))
    ...
    def run_episode(self, ep, training=True, N_fleet_override=None,
        ...
        if (learned_training and not lower_critic_frozen
            ...
            for _ in range(self.updates_per_episode):
                ...
                if lower_m.get('regularity_projection_applied', 0.0) == 1.0:
        ...
        if lower_projection_updates:
            ...
            for metric in projection_episode_metrics:
                values = np.asarray([
                    float(update.get(metric, 0.0))
        ...
        lower_regularity_policy_capacity_gate_stat = _stat(
            self._ep_lower_regularity_policy_capacity_gates)
        lower_regularity_policy_action_efficiency_gate_stat = _stat(
            self._ep_lower_regularity_policy_action_efficiency_gates)
        lower_regularity_policy_fleet_utilization_stat = _stat(
            self._ep_lower_regularity_policy_fleet_utilizations)
        lower_regularity_policy_fleet_pressure_stat = _stat(
            self._ep_lower_regularity_policy_fleet_pressures)
        ...
        lower_regularity_policy_hf_energy_pressure_stat = _stat(
            self._ep_lower_regularity_policy_hf_energy_pressures)
        lower_regularity_gain_floor_required_fraction_stat = _stat(
            self._ep_lower_regularity_gain_floor_required_fractions)
        lower_regularity_gain_floor_hf_pressure_stat = _stat(
            self._ep_lower_regularity_gain_floor_hf_pressures)
        lower_regularity_gain_floor_expected_gain_fraction_stat = _stat(
            self._ep_lower_regularity_gain_floor_expected_gain_fractions)
        lower_regularity_gain_floor_selected_gain_fraction_stat = _stat(
            self._ep_lower_regularity_gain_floor_selected_gain_fractions)
        lower_regularity_gain_floor_expected_shortfall_stat = _stat(
            self._ep_lower_regularity_gain_floor_expected_shortfalls)
        ...
        row = {
            ...
            'lower_regularity_projection_distillation': str(
                self.lower_trainer.regularity_projection_distillation),
            'lower_regularity_projection_distillation_steps': int(
                self.lower_trainer
                .regularity_projection_distillation_steps),
            ...
            'lower_regularity_projection_actor_forward_kl': lower_m.get(
                'regularity_projection_actor_forward_kl', 0.0),
            'lower_regularity_projection_actor_distillation_steps': lower_m.get(
                'regularity_projection_actor_distillation_steps', 0.0),
            'lower_regularity_projection_actor_post_reverse_kl': lower_m.get(
                'regularity_projection_actor_post_reverse_kl', 0.0),
            'lower_regularity_projection_actor_post_forward_kl': lower_m.get(
                'regularity_projection_actor_post_forward_kl', 0.0),
            'lower_regularity_projection_actor_post_regularity_cost': lower_m.get(
                'regularity_projection_actor_post_regularity_cost', 0.0),
            'lower_regularity_projection_actor_post_passenger_cost': lower_m.get(
                'regularity_projection_actor_post_passenger_cost', 0.0),
            'lower_regularity_projection_actor_post_action_mean_s': lower_m.get(
            ...
            'lower_regularity_projection_actor_post_reverse_kl_episode_max': (
                lower_m.get(
                    'regularity_projection_actor_post_reverse_kl_episode_max',
                    0.0)),
            'lower_regularity_projection_actor_post_forward_kl_episode_mean': (
                lower_m.get(
                    'regularity_projection_actor_post_forward_kl_episode_mean',
                    0.0)),
            'lower_regularity_projection_actor_post_forward_kl_episode_max': (
                lower_m.get(
                    'regularity_projection_actor_post_forward_kl_episode_max',
                    0.0)),
            'lower_regularity_projection_actor_post_regularity_cost_episode_mean': (
                lower_m.get(
                    'regularity_projection_actor_post_regularity_cost_episode_mean',
                    0.0)),
            'lower_regularity_projection_actor_post_regularity_cost_episode_max': (
                lower_m.get(
                    'regularity_projection_actor_post_regularity_cost_episode_max',
                    0.0)),
            'lower_regularity_projection_actor_post_passenger_cost_episode_mean': (
                lower_m.get(
                    'regularity_projection_actor_post_passenger_cost_episode_mean',
                    0.0)),
            'lower_regularity_projection_actor_post_passenger_cost_episode_max': (
                lower_m.get(
                    'regularity_projection_actor_post_passenger_cost_episode_max',
                    0.0)),
            'lower_regularity_projection_actor_post_target_action_change_mean_s_episode_mean': (
                lower_m.get(
                    'regularity_projection_actor_post_target_action_change_mean_s_episode_mean',
                    0.0)),
            'lower_regularity_projection_actor_post_constraints_met_episode_mean': (
                lower_m.get(
            ...
            'lower_regularity_policy_capacity_gain_mode': str(
                self.lower_trainer.regularity_capacity_gain_mode),
            ...
            'lower_regularity_policy_capacity_exponent': float(
                self.lower_trainer.regularity_capacity_exponent),
            'lower_regularity_policy_action_efficiency_penalty': float(
                self.lower_trainer
                .regularity_capacity_action_efficiency_penalty),
            'lower_regularity_policy_fleet_pressure_start': float(
                self.lower_trainer.regularity_capacity_fleet_pressure_start),
            'lower_regularity_policy_fleet_pressure_full': float(
                self.lower_trainer.regularity_capacity_fleet_pressure_full),
            'lower_regularity_policy_fleet_pressure_exponent': float(
                self.lower_trainer.regularity_capacity_fleet_pressure_exponent),
            'lower_regularity_policy_opportunity_cost_penalty': float(
                self.lower_trainer
                .regularity_capacity_opportunity_cost_penalty),
            ...
            'lower_regularity_policy_actor_action_efficiency_gate_mean': (
                lower_m.get(
                    'regularity_policy_action_efficiency_gate_mean', 0.)),
            'lower_regularity_policy_actor_fleet_utilization_mean': (
                lower_m.get(
                    'regularity_policy_fleet_utilization_mean', 0.)),
            'lower_regularity_policy_actor_fleet_pressure_mean': (
            ...
            'lower_regularity_policy_action_efficiency_gate_mean': round(
                lower_regularity_policy_action_efficiency_gate_stat['mean'],
                8),
            'lower_regularity_policy_fleet_utilization_mean': round(
                lower_regularity_policy_fleet_utilization_stat['mean'], 8),
            'lower_regularity_policy_fleet_pressure_mean': round(
            ...
            'lower_regularity_gain_floor_enabled': int(
                self.lower_trainer.regularity_gain_floor_enabled),
            'lower_regularity_gain_floor_mode': str(
                self.lower_trainer.regularity_gain_floor_mode),
            'lower_regularity_gain_floor_base_fraction': float(
                self.lower_trainer.regularity_gain_floor_base_fraction),
            'lower_regularity_gain_floor_hf_increment': float(
                self.lower_trainer.regularity_gain_floor_hf_increment),
            'lower_regularity_gain_floor_hf_energy_scale': float(
                self.lower_trainer.regularity_gain_floor_hf_energy_scale),
            'lower_regularity_gain_floor_hf_energy_exponent': float(
                self.lower_trainer.regularity_gain_floor_hf_energy_exponent),
            'lower_regularity_gain_floor_actor_required_fraction_mean': (
                lower_m.get(
                    'regularity_gain_floor_required_fraction_mean', 0.0)),
            'lower_regularity_gain_floor_actor_hf_energy_mean': lower_m.get(
                'regularity_gain_floor_hf_energy_mean', 0.0),
            'lower_regularity_gain_floor_actor_hf_pressure_mean': lower_m.get(
            ...
            'lower_regularity_gain_floor_required_fraction_mean': round(
                lower_regularity_gain_floor_required_fraction_stat['mean'], 8),
            'lower_regularity_gain_floor_hf_pressure_mean': round(
                lower_regularity_gain_floor_hf_pressure_stat['mean'], 8),
            'lower_regularity_gain_floor_expected_gain_fraction_mean': round(
                lower_regularity_gain_floor_expected_gain_fraction_stat['mean'],
                8),
            'lower_regularity_gain_floor_selected_gain_fraction_mean': round(
                lower_regularity_gain_floor_selected_gain_fraction_stat['mean'],
                8),
            'lower_regularity_gain_floor_expected_shortfall_mean': round(
                lower_regularity_gain_floor_expected_shortfall_stat['mean'], 8),
            'lower_regularity_gain_floor_selected_shortfall_mean': round(
            ...
            'lower_regularity_policy_target_action_mean_s': round(
                lower_regularity_policy_target_stat['mean'], 6),
            'lower_regularity_policy_abs_error_mean_s': round(
                lower_regularity_policy_error_stat['mean'], 6),
