#!/usr/bin/env python3
"""Submit Stage-8 plan-value qualification to the Linux CPU pool."""
...
from scripts import pointmaze_plan_value_stage8_spec as spec  # noqa: E402
...
CPU_JUSTIFICATION = (
    "Independent hidden-regime PointMaze PPO roots are single-threaded and "
...
def task_signature(
    ...
    return (
        f"Freq-HRL/{spec.EXPERIMENT_PROTOCOL}/{run_name}/{method}/"
...
def training_command(
    ...
    *,
    ...
    command = [
        ...
        str(options["waypoint_perturbation"]),
        "--event-window-seconds",
        str(options["event_window_seconds"]),
...
def task_specification(
    ...
    *,
    ...
    return {
        ...
        "description": (
            f"Freq-HRL PointMaze stage8 {phase} {method} "
...
def sync_results(
    ...
    *,
    ...
    for method, optimizer_seed in spec.cells(preflight=preflight):
        ...
        if task is None:
            raise SystemExit(f"Stage-8 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"Stage-8 task is not done: {task.get('id')} "
    ...
    if pending:
        ...
        raise SystemExit(
            f"Stage-8 result sync incomplete: {len(pending)} cells; "
    ...
    for signature, path, _ in expected:
        ...
        if (
            ...
            raise SystemExit(f"invalid synced Stage-8 result: {signature}")
    ...
    print(f"synced {len(expected)} Stage-8 result JSON files")
