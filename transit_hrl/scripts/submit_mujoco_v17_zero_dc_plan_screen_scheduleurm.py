#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v17 raw-action screen."""
...
SIGNATURE_VERSION = "mujoco-v17-zero-dc-plan-screen-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": f"Freq-HRL v17 {environment} {arm} {optimizer_seed}",
        ...
        "cpu_training_justification": (
            "Independent MuJoCo architecture cells are CPU-bound and single-threaded."
        ),
...
def _scheduler_tasks(run_name: str) -> dict[str, dict[str, Any]]:
    ...
    for signature, attempts in grouped.items():
        ...
        if len(done) > 1:
            raise RuntimeError(f"multiple successful scheduler attempts for {signature}")
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v17 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v17 task is not done: {task.get('id')} status={task.get('status')}"
            )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v17 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v17 small-result cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v17 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v17 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v17 environment subset")
    if (
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v17 frozen algorithm identity mismatch")
    if args.sync_only:
        sync_results(args)
        return
