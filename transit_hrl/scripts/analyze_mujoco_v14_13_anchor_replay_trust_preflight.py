#!/usr/bin/env python3
"""Audit the scoped single-seed MuJoCo v14.13 preflight."""
...
ANALYSIS_VERSION = "mujoco_v14_13_anchor_replay_trust_preflight_analysis_v1"
FREQUENCY_METRICS = (
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.13 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    ...
    if len(set(keys)) != len(keys) or set(keys) != expected_keys:
        raise ValueError(f"v14.13 evaluation path registry mismatch: {path}")
    return rows
...
def _load_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise ValueError(f"invalid v14.13 preflight cell: {path}")
...
def _mode_payload(rows: list[dict[str, str]], mode: str) -> dict[str, Any]:
    ...
    if len(selected) != len(spec.DEVELOPMENT_EVALUATION_SEEDS):
        raise ValueError(f"v14.13 preflight mode is incomplete: {mode}")
    payload = {
        metric: (
...
def analyze_preflight(
    ...
    if environment not in spec.ENVIRONMENTS:
        raise ValueError(f"invalid v14.13 preflight environment: {environment}")
    if int(optimizer_seed) not in spec.OPTIMIZER_SEEDS:
        raise ValueError("invalid v14.13 preflight optimizer seed")
    ...
    identity_pass = bool(
        ...
        == "frozen_before_v14_13_anchor_replay_trust_outcome_access"
    ...
    if not identity_pass:
        raise ValueError("v14.13 preflight scope or frozen identity mismatch")
    ...
    if not (
        ...
        raise ValueError("v14.13 preflight anchor identity mismatch")
    ...
    for arm in spec.ARMS:
        ...
        if not (
            ...
            raise ValueError(f"v14.13 preflight cell identity mismatch: {arm}")
        ...
        if not (
            ...
            raise ValueError(f"v14.13 preflight anchor provenance mismatch: {arm}")
        if (
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.13 preflight paired baseline mismatch: {arm}"
                )
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.13 checkpoint diagnostics are incomplete: {arm}"
                )
        cells[arm] = cell
    ...
    for arm in spec.LEARNED_ARMS:
        ...
        arm_status[arm] = {
            ...
            "selected_checkpoint_diagnostics": summary.get(
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_13_preflight status={decision['status']} "
