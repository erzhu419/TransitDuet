#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.10 deployment-aligned screen."""
...
from scripts import mujoco_v14_10_deployment_aligned_screen_spec as spec  # noqa: E402
...
SIGNATURE_VERSION = "mujoco-v14-10-deployment-aligned-screen-v1"
ANCHOR_ARM = "deployment_aligned_pretrain_anchor"
...
def runtime_identity() -> dict[str, str]:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v14.10 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14.10-Deployment-Aligned-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14.10 {phase} {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        "status": "frozen_before_v14_10_deployment_aligned_outcome_access",
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14.10 preregistration differs")
...
def _read_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise SystemExit(f"v14.10 cell payload is invalid: {path}")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if not sync_manifest_path.is_file():
        raise SystemExit("v14.10 merge requires run-scoped result sync")
    ...
    if (
        ...
        raise SystemExit("v14.10 run-scoped result sync manifest is invalid")
    expected = [
        (
            phase,
            environment,
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14.10 screen: {len(missing)} cells missing; first={missing[0]}"
        )
    ...
    if issues:
        raise SystemExit("v14.10 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14.10 cells")
...
def _scheduler_tasks_for_run(run_name: str) -> dict[str, dict[str, Any]]:
    ...
    if completed is None or completed.returncode != 0:
        raise RuntimeError(
            "v14.10 scheduler snapshot failed after three attempts: "
    ...
    for task in payload.get("tasks", []):
        ...
        if signature in matches:
            raise RuntimeError(
                f"duplicate scheduler signature during v14.10 sync: {signature}"
            )
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for phase, environment, arm, seed in cells:
        ...
        if task is None:
            raise SystemExit(f"v14.10 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v14.10 sync task is not done: {task.get('id')} "
    ...
    print(
        f"synced {len(expected)} MuJoCo v14.10 cells with "
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v14.10 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14.10 screen arm registry")
    if not args.phases or not set(args.phases).issubset(PHASES):
        raise SystemExit("invalid v14.10 screen phase registry")
    if (
        not args.environments
        or not set(args.environments).issubset(spec.ENVIRONMENTS)
    ):
        raise SystemExit("invalid v14.10 environment subset")
    if (
        ...
        raise SystemExit("invalid v14.10 optimizer seed subset")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14.10 screen nodes: {unknown_nodes}")
    ...
    if not 1 <= int(args.sync_workers) <= 8:
        raise SystemExit("v14.10 sync workers must be in [1, 8]")
    return args
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14.10 frozen algorithm identity mismatch")
    ...
    if not cells:
        print("no v14.10 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14.10 deployment-aligned screen {args.run_name}"
    ))
