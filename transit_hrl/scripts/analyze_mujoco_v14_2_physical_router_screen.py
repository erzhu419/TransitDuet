#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v14.2 screen as development evidence."""
...
ANALYSIS_VERSION = "mujoco_v14_2_physical_router_screen_analysis_v1"
METRICS = (
    "episode_return",
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.2 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    path_keys = [
    ...
    if len(set(path_keys)) != len(path_keys) or set(path_keys) != expected_keys:
        raise ValueError(f"v14.2 evaluation path registry mismatch: {path}")
    return rows
...
def _finite_mean(rows: list[dict[str, str]], metric: str) -> float:
    ...
    if values.size == 0 or not np.all(np.isfinite(values)):
        raise ValueError(f"non-finite v14.2 metric: {metric}")
...
def _bounds(
    ...
    *,
    ...
    if array.size < 2 or not np.all(np.isfinite(array)):
        raise ValueError("v14.2 bootstrap requires finite optimizer replicates")
...
def load_replicates(
    ...
    if (
        ...
        != "frozen_before_v14_2_development_outcome_access"
        ...
        raise ValueError("v14.2 preregistration identity mismatch")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("v14.2 analysis requires a complete merged manifest")
        ...
        for environment in spec.ENVIRONMENTS:
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not isinstance(history, list) or not history:
                    raise ValueError("v14.2 training history is empty")
                ...
                if (
                    ...
                    raise ValueError("v14.2 dual history contains non-finite values")
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "The v14.2 screen may select one fully trained, non-saturating "
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    if decision_path.exists() and decision_path.read_text(
        ...
        raise RuntimeError("existing v14.2 screen decision differs")
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_2_screen status={decision['status']} "
