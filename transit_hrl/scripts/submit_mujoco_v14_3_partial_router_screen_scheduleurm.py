#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.3 development screen via scheduleurm."""
...
SIGNATURE_VERSION = "mujoco-v14-3-partial-router-screen-v1"
...
def runtime_identity() -> dict[str, str]:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v14.3 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14.3-Partial-Router-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14.3 {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        "status": "frozen_before_v14_3_development_outcome_access",
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14.3 preregistration differs")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14.3 screen: {len(missing)} cells missing; "
            f"first={missing[0]}"
        )
    issues: list[str] = []
    cells = []
    ...
    if issues:
        raise SystemExit("v14.3 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14.3 development cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14.3 screen arm registry")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14.3 screen nodes: {unknown_nodes}")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14.3 frozen algorithm identity mismatch")
    ...
    if not cells:
        print("no v14.3 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14.3 partial-router screen {args.run_name}"
    ))
