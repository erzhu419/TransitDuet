#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v17.2 paired smooth-gauge preflight."""
...
def _validate_preregistration(root: Path, run_name: str) -> None:
    ...
    if (
        ...
        or preregistration.get("alpha_arms") != spec.ALPHA_ARMS
        ...
        raise ValueError("v17.2 preregistration or result-sync contract mismatch")
...
def _split_interventions(
    ...
    if (
        ...
        raise ValueError("v17.2 paired intervention row counts are incomplete")
...
def _validate_cell(
    ...
    alpha = float(spec.ALPHA_ARMS[str(alpha_arm)])
    if (
        ...
        or summary.get("alpha_arm") != str(alpha_arm)
        or float(summary.get("paired_intervention_alpha", -1.0)) != alpha
        ...
        or float(summary.get("lower_action_router_alpha", -1.0)) != alpha
        ...
        raise ValueError(
            ...
            f"{environment}/{alpha_arm}/{optimizer_seed}"
        )
    ...
    if _path_registry(control) != expected_paths or _path_registry(candidate) != expected_paths:
        raise ValueError(
            f"v17.2 heldout path registry mismatch: {environment}/{alpha_arm}"
        )
    if any(
        ...
        raise ValueError("v17.2 paired intervention strength mismatch")
    audit = paired_intervention_audit(control, candidate)
    if audit != summary.get("paired_intervention_audit"):
        raise ValueError("v17.2 stored paired audit does not reproduce")
...
def analyze(run_name: str, *, root: Path = ROOT) -> dict[str, Any]:
    _validate_preregistration(Path(root), str(run_name))
    cells: list[dict[str, Any]] = []
    for environment in spec.ENVIRONMENTS:
        for alpha_arm in spec.ALPHA_ARMS:
            for optimizer_seed in spec.OPTIMIZER_SEEDS:
                summary, rows = _load_cell(
                    Path(root), run_name, environment, alpha_arm, optimizer_seed
                )
                ...
                cells.append({
                    ...
                    "alpha_arm": alpha_arm,
                    "alpha": float(spec.ALPHA_ARMS[alpha_arm]),
    ...
    for alpha_arm, alpha in spec.ALPHA_ARMS.items():
        rows = [row for row in cells if row["alpha_arm"] == alpha_arm]
        alpha_results.append({
            "alpha_arm": alpha_arm,
            "alpha": float(alpha),
            "eligible_for_leakage_active_multiseed": bool(
                len(rows) == len(spec.ENVIRONMENTS)
                and all(bool(row["supported"]) for row in rows)
            ),
            "supported_environment_count": sum(
                bool(row["supported"]) for row in rows
            ),
            "worst_environment_joint_merit_relative_reduction": min(
                float(row["joint_merit_relative_reduction"]) for row in rows
            ),
            "median_joint_merit_relative_reduction": statistics.median(
                float(row["joint_merit_relative_reduction"]) for row in rows
            ),
        })
    eligible = [
        row for row in alpha_results
        if row["eligible_for_leakage_active_multiseed"]
    ]
    selected = (
        max(eligible, key=lambda row: (
            float(row["worst_environment_joint_merit_relative_reduction"]),
            float(row["median_joint_merit_relative_reduction"]),
        ))
        if eligible else None
    )
    return {
        "status": (
            spec.SUPPORTED_STATUS if selected is not None
            else spec.NOT_SUPPORTED_STATUS
        ),
        ...
        "selected_alpha_arm": (
            None if selected is None else selected["alpha_arm"]
        ),
...
def main() -> None:
    ...
    (output / "smooth_macro_gauge_preflight.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    _write_csv(output / "smooth_macro_gauge_cells.csv", result["cells"])
    print(json.dumps({
        "status": result["status"],
        "selected_alpha_arm": result["selected_alpha_arm"],
