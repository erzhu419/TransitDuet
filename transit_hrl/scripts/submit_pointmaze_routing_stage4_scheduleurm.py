#!/usr/bin/env python3
"""Submit PointMaze Stage-4 routing-attribution cells to the Linux CPU pool."""
...
from scripts import pointmaze_routing_stage4_spec as spec  # noqa: E402
...
CPU_JUSTIFICATION = (
    "Independent PointMaze routing-attribution cells use single-thread CPU "
    "policies and are dynamically distributed across the physical-core pool."
)
    ...
    *,
    ...
    for scenario, method, optimizer_seed in protocol_spec.cells(
        ...
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"stage-4 task is not done: {task.get('id')} "
    ...
    if pending:
        ...
        raise SystemExit(
            f"stage-4 result sync incomplete: {len(pending)} cells; "
