#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.4 development screen via scheduleurm."""
...
from scripts import mujoco_v14_4_router_homotopy_screen_spec as spec  # noqa: E402
...
SIGNATURE_VERSION = "mujoco-v14-4-router-homotopy-screen-v1"
...
def runtime_identity() -> dict[str, str]:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v14.4 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14.4-Router-Homotopy-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14.4 {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        ...
        "checkpoint_score_contract": "arm_specific_frozen_in_arms",
        ...
        "development_selection_gates": {
            ...
            "maximum_dual_saturation_fraction": (
                spec.MAXIMUM_DUAL_SATURATION_FRACTION
            ),
            "dual_saturation_threshold": spec.DUAL_SATURATION_THRESHOLD,
            "minimum_strict_responsibility_improvement_conditions": (
                spec.MINIMUM_STRICT_RESPONSIBILITY_IMPROVEMENT_CONDITIONS
            ),
            "minimum_strict_raw_improvement_conditions": (
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14.4 preregistration differs")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14.4 screen: {len(missing)} cells missing; "
            f"first={missing[0]}"
        )
    issues: list[str] = []
    cells = []
    ...
    for environment, arm, seed, path in expected:
        ...
        history = json.loads(
            (path / "training_history.json").read_text(encoding="utf-8")
        )
        if not isinstance(history, list) or not history:
            raise SystemExit(f"v14.4 training history is invalid: {path}")
        with (path / "evaluation_rows.csv").open(
        ...
        expected_training_strengths = (
            spec.expected_router_training_strengths(arm)
        )
        reported_training_strengths = summary.get(
            "lower_action_router_training_strengths_by_iteration", []
        )
        sampled_training_strengths = [
            float(row.get(
                "sampled_lower_action_router_strength", float("nan")
            ))
            for row in history[1:]
        ]
        reported_selection_strengths = [
            float(row["LowerActionRouterStrength_mean"])
            for row in history
            if "LowerActionRouterStrength_mean" in row
        ]
        observed_evaluation_grid = {
            (str(row["disturbance_mode"]), int(row["seed"]))
            for row in evaluation_rows
        }
        expected_evaluation_grid = {
            (mode, int(evaluation_seed))
        ...
        checks = {
            ...
            "lower_dual": float(summary.get(
                "lower_dual_lr", float("nan")
            )) == float(arm_spec["lower_dual_lr"]),
            "cost_mode": summary.get("leakage_constraint_cost_mode")
            == arm_spec["leakage_cost_mode"],
            "router_mode": summary.get("lower_action_router_mode")
            == arm_spec["lower_action_router_mode"],
            "router_alpha": float(summary.get(
                "lower_action_router_alpha", float("nan")
            )) == float(arm_spec["lower_action_router_alpha"]),
            "router_strength": float(summary.get(
                "lower_action_router_strength", float("nan")
            ...
            "router_warmup": float(summary.get(
                "lower_action_router_warmup_fraction", float("nan")
            )) == float(arm_spec["lower_action_router_warmup_fraction"]),
            "router_ramp": float(summary.get(
                "lower_action_router_ramp_fraction", float("nan")
            )) == float(arm_spec["lower_action_router_ramp_fraction"]),
            "router_strength_observed": bool(summary.get(
                "lower_action_router_observe_strength", False
            )) == bool(arm_spec["lower_action_router_observe_strength"]),
            "router_schedule_payload": (
                len(reported_training_strengths)
                == len(expected_training_strengths)
                and all(
                abs(float(actual) - expected) <= 1e-12
                for actual, expected in zip(
                    reported_training_strengths,
                    expected_training_strengths,
                )
                )
            ),
            "router_training_history": (
                len(sampled_training_strengths)
                == len(expected_training_strengths)
                and all(
            ...
            "router_selection_target": (
                bool(reported_selection_strengths)
                and all(
                    abs(value - target_strength) <= 1e-12
                    for value in reported_selection_strengths
                )
            ),
            ...
            "checkpoint_penalty": float(summary.get(
            ...
            "evaluation_file_rows": len(evaluation_rows) == expected_rows,
            "evaluation_grid": observed_evaluation_grid
            == expected_evaluation_grid,
            "evaluation_target_strength": all(
                abs(float(row["LowerActionRouterStrength"])
                    - target_strength) <= 1e-12
                for row in evaluation_rows
            ),
            ...
            == "verified",
            "checkpoint_hash": _sha256(path / "checkpoint.pt")
    ...
    if issues:
        raise SystemExit("v14.4 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14.4 development cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14.4 screen arm registry")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14.4 screen nodes: {unknown_nodes}")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14.4 frozen algorithm identity mismatch")
    ...
    if not cells:
        print("no v14.4 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14.4 homotopy screen {args.run_name}"
    ))
