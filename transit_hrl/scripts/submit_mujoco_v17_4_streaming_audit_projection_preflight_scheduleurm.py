#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v17.4 paired preflight."""
...
WORKER = Path("scripts/run_mujoco_v17_4_paired_gauge_cell.py")
SIGNATURE_VERSION = "mujoco-v17-4-streaming-audit-projection-preflight-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": (
            f"Freq-HRL v17.4 {environment} {optimizer_seed}"
        ),
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v17.4 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                "v17.4 task is not done: "
    ...
    if pending:
        ...
        raise SystemExit(
            f"v17.4 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v17.4 small-result cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v17.4 optimizer seed subset") from exc
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v17.4 environment subset")
    if (
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v17.4 frozen algorithm identity mismatch")
    if args.sync_only:
        sync_results(args)
        return
