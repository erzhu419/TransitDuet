#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.8 latent matched-control screen."""
...
SIGNATURE_VERSION = "mujoco-v14-8-latent-matched-screen-v1"
ANCHOR_ARM = "latent_zero_projection_pretrain_anchor"
...
def runtime_identity() -> dict[str, str]:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v14.8 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14.8-Latent-Matched-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14.8 {phase} {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        "status": "frozen_before_v14_8_development_outcome_access",
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14.8 preregistration differs")
...
def _read_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise SystemExit(f"v14.8 cell payload is invalid: {path}")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if not sync_manifest_path.is_file():
        raise SystemExit("v14.8 merge requires run-scoped result sync")
    ...
    if (
        ...
        raise SystemExit("v14.8 run-scoped result sync manifest is invalid")
    expected = [
        (
            phase,
            environment,
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14.8 screen: {len(missing)} cells missing; first={missing[0]}"
        )
    ...
    if issues:
        raise SystemExit("v14.8 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14.8 cells")
...
def _scheduler_tasks_for_run(run_name: str) -> dict[str, dict[str, Any]]:
    ...
    if completed is None or completed.returncode != 0:
        raise RuntimeError(
            "v14.8 scheduler snapshot failed after three attempts: "
    ...
    for task in payload.get("tasks", []):
        ...
        if signature in matches:
            raise RuntimeError(
                f"duplicate scheduler signature during v14.8 sync: {signature}"
            )
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for phase, environment, arm, seed in cells:
        ...
        if task is None:
            raise SystemExit(f"v14.8 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v14.8 sync task is not done: {task.get('id')} "
    ...
    print(
        f"synced {len(expected)} MuJoCo v14.8 cells with "
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14.8 screen arm registry")
    if not args.phases or not set(args.phases).issubset(PHASES):
        raise SystemExit("invalid v14.8 screen phase registry")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14.8 screen nodes: {unknown_nodes}")
    ...
    if not 1 <= int(args.sync_workers) <= 8:
        raise SystemExit("v14.8 sync workers must be in [1, 8]")
    return args
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14.8 frozen algorithm identity mismatch")
    ...
    if int(args.max_cells) > 0:
        cells = cells[:int(args.max_cells)]
    if not cells:
        print("no v14.8 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14.8 latent matched-control screen {args.run_name}"
    ))
