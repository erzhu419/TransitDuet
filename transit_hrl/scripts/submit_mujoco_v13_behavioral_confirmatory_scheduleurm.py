#!/usr/bin/env python3
"""Submit the frozen MuJoCo v13 behavioral confirmatory matrix through scheduleurm."""
...
RUNTIME_PATH = Path(__file__).with_name("mujoco_v13_behavioral_confirmatory_runtime.py")
SPEC_PATH = Path(spec.__file__).resolve()
SIGNATURE_VERSION = "mujoco-v13-behavioral-confirmatory-v1"
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-Behavioral-Confirmatory",
        "description": (
            f"Freq-HRL MuJoCo v13 behavioral {args.arm} {environment} replicate "
            f"{optimizer_seed}"
        ),
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        ...
        "status": "frozen_before_v13_heldout_access",
        ...
        "primary_gates": {
            ...
            "maximum_reconstruction_rms_integrity_gate": (
        ...
        "metric_definitions": {
            "responsibility_drift": "mean held-out LowerLFDriftAbs",
            "raw_lower_drift": "mean held-out RawLowerLFDriftAbs",
            "upper_hf_rms": "sqrt(mean held-out UpperHFPowerAbs)",
            "return": "mean held-out episode_return",
        },
    }
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if missing:
        raise SystemExit(
            f"cannot merge MuJoCo v13 behavioral confirmatory arm: {len(missing)} cells "
    ...
    print(f"merged {len(summaries)} frozen MuJoCo v13 behavioral cells into {output}")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("frozen MuJoCo v13 behavioral algorithm identity mismatch")
    ...
    if not cells:
        print("no frozen MuJoCo v13 behavioral cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v13 behavioral confirmatory {args.arm}"
    ))
