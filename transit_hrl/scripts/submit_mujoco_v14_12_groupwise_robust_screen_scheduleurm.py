#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.12 groupwise-robust screen."""
...
from scripts import mujoco_v14_12_groupwise_robust_screen_spec as spec  # noqa: E402
...
SIGNATURE_VERSION = "mujoco-v14-12-groupwise-robust-screen-v1"
ANCHOR_ARM = "groupwise_robust_pretrain_anchor"
...
def runtime_identity() -> dict[str, str]:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v14.12 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14.12-Groupwise-Robust-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14.12 {phase} {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        "status": "frozen_before_v14_12_groupwise_robust_outcome_access",
        ...
        "pooled_comparator_arm": spec.POOLED_COMPARATOR_ARM,
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14.12 preregistration differs")
...
def _read_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise SystemExit(f"v14.12 cell payload is invalid: {path}")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if not sync_manifest_path.is_file():
        raise SystemExit("v14.12 merge requires run-scoped result sync")
    ...
    if (
        ...
        raise SystemExit("v14.12 run-scoped result sync manifest is invalid")
    expected = [
        (
            phase,
            environment,
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14.12 screen: {len(missing)} cells missing; first={missing[0]}"
        )
                ...
                "deployment_projection_contract": summary.get(
                    ...
                    (
                        ...
                        if bool(arm_spec[
                            ...
                            "episode_reset_differentiable_actor_mean_tanh_"
                            "upper_hold_hpf8_lower_lpf32_anchor_relative_"
                            "target_with_absolute_floor_and_dimensionless_"
                            "iterative_cumulative_reward_budget_projection_v4"
                        )
                    )
                ) if (
    ...
    if issues:
        raise SystemExit("v14.12 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14.12 cells")
...
def _scheduler_tasks_for_run(run_name: str) -> dict[str, dict[str, Any]]:
    ...
    if completed is None or completed.returncode != 0:
        raise RuntimeError(
            "v14.12 scheduler snapshot failed after three attempts: "
    ...
    for task in payload.get("tasks", []):
        ...
        if signature in matches:
            raise RuntimeError(
                f"duplicate scheduler signature during v14.12 sync: {signature}"
            )
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for phase, environment, arm, seed in cells:
        ...
        if task is None:
            raise SystemExit(f"v14.12 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v14.12 sync task is not done: {task.get('id')} "
    ...
    print(
        f"synced {len(expected)} MuJoCo v14.12 cells with "
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v14.12 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14.12 screen arm registry")
    if not args.phases or not set(args.phases).issubset(PHASES):
        raise SystemExit("invalid v14.12 screen phase registry")
    if (
        not args.environments
        or not set(args.environments).issubset(spec.ENVIRONMENTS)
    ):
        raise SystemExit("invalid v14.12 environment subset")
    if (
        ...
        raise SystemExit("invalid v14.12 optimizer seed subset")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14.12 screen nodes: {unknown_nodes}")
    ...
    if not 1 <= int(args.sync_workers) <= 8:
        raise SystemExit("v14.12 sync workers must be in [1, 8]")
    return args
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14.12 frozen algorithm identity mismatch")
    ...
    if not cells:
        print("no v14.12 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14.12 groupwise-robust screen {args.run_name}"
    ))
