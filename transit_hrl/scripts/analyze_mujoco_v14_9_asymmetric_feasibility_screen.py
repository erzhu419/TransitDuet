#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v14.9 asymmetric feasibility screen."""
...
from scripts import mujoco_v14_9_asymmetric_feasibility_screen_spec as spec  # noqa: E402


ANALYSIS_VERSION = "mujoco_v14_9_asymmetric_feasibility_screen_analysis_v1"
METRICS = (
    "episode_return",
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.9 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    ...
    if len(set(keys)) != len(keys) or set(keys) != expected_keys:
        raise ValueError(f"v14.9 evaluation path registry mismatch: {path}")
    return rows
...
def _finite_mean(rows: list[dict[str, str]], metric: str) -> float:
    ...
    if values.size == 0 or not np.all(np.isfinite(values)):
        raise ValueError(f"non-finite v14.9 metric: {metric}")
...
def _bounds(
    ...
    *,
    ...
    if array.size < 2 or not np.all(np.isfinite(array)):
        raise ValueError("v14.9 bootstrap requires finite optimizer replicates")
...
def _actor_vectors(path: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not isinstance(model, dict):
        raise ValueError(f"invalid v14.9 checkpoint payload: {path}")
    vectors: list[np.ndarray] = []
    for actor_name in ("upper_actor", "lower_actor"):
        actor = model.get(actor_name)
        if not isinstance(actor, dict) or not actor:
            raise ValueError(f"missing v14.9 checkpoint actor {actor_name}: {path}")
        values = []
        for name in sorted(actor):
            tensor = actor[name]
            if not torch.is_tensor(tensor):
                raise ValueError(
                    f"non-tensor v14.9 actor parameter {actor_name}.{name}"
                )
            values.append(
...
def _actor_rms_difference(candidate: Path, comparator: Path) -> dict[str, float]:
    ...
    if (
        ...
        raise ValueError("v14.9 paired actor checkpoint shapes differ")
...
def load_replicates(
    ...
    if (
        ...
        != "frozen_before_v14_9_development_outcome_access"
        ...
        raise ValueError("v14.9 preregistration identity mismatch")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("v14.9 analysis requires a complete merged manifest")
        ...
        for arm, arm_spec in spec.ARMS.items():
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not np.allclose(
                    ...
                    raise ValueError(
                        f"v14.9 router strength mismatch: {environment}/{arm}/{seed}"
                    )
                for mode in spec.EVALUATION_DISTURBANCE_MODES:
                    mode_rows = [
                    ...
                    for trace in TRACE_KEYS:
                        ...
                        if any(len(value) != 64 for value in values):
                            raise ValueError(f"invalid v14.9 trace hash: {trace}")
        ...
        for environment in spec.ENVIRONMENTS:
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not isinstance(history, list) or not history:
                    raise ValueError("v14.9 training history is empty")
                ...
                if len(parameter_hash) != 64:
                    raise ValueError("v14.9 selected parameter hash is invalid")
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "The v14.9 projection calibration is compared with a mean-reward "
            ...
            "and lower dual rates and is compared with the same-strength, "
            "state-aligned feasibility-selector zero-dual control. It must "
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    if decision_path.exists() and decision_path.read_text(
        ...
        raise RuntimeError("existing v14.9 screen decision differs")
    ...
    lines = [
        "# MuJoCo v14.9 Asymmetric Feasibility Screen",
        "",
        ...
        "- Learned comparator: same projection and state-aligned feasibility selector, zero dual.",
        "",
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_9_screen status={decision['status']} "
