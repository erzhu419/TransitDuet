#!/usr/bin/env python3
"""Submit v17.6 full-horizon oracle paths through scheduleurm."""
...
from scripts.analyze_mujoco_v17_6_full_horizon_oracle import (  # noqa: E402
    path_relative_dir,
)
...
WORKER = Path("scripts/run_mujoco_v17_6_full_horizon_oracle_path.py")
SIGNATURE_VERSION = "mujoco-v17-6-full-horizon-oracle-v1"
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": f"Freq-HRL v17.6 oracle {environment} {mode} {seed}",
...
def _write_registration(args: argparse.Namespace) -> None:
    ...
    payload = {
        "status": "registered_before_oracle_path_access",
        ...
        "solver_contract": {
            "solver": "scipy.optimize.lsq_linear_bvls",
        ...
        "scheduler_contract": {
            ...
            "require_node": DATA_LOCAL_NODE,
            "binding_reason": "source checkpoints remain server-only on node003",
            "cpu_per_path": 1,
            "ram_mb_per_path": 1024,
            "slurm_used": False,
        },
        "claim_boundary": (
            "reused rejected paths; oracle may select a mechanism but cannot "
...
def main() -> None:
    ...
    if not args.environments or not set(args.environments).issubset(
        ...
        raise SystemExit("invalid v17.6 environment subset")
    if not args.disturbance_modes or not set(args.disturbance_modes).issubset(
        spec.DISTURBANCE_MODES
    ):
        raise SystemExit("invalid v17.6 disturbance subset")
    if not args.evaluation_seeds or not set(args.evaluation_seeds).issubset(
        spec.EVALUATION_SEEDS
    ):
        raise SystemExit("invalid v17.6 evaluation-seed subset")
    revision, manifest = source_identity(spec.FROZEN_ORACLE_REVISION)
    if (
        revision != spec.FROZEN_ORACLE_REVISION
        or manifest != spec.FROZEN_SOURCE_MANIFEST_SHA256
    ):
        raise SystemExit("v17.6 frozen oracle identity mismatch")
