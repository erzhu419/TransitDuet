#!/usr/bin/env python3
"""Submit the frozen v18.4 causal receding-horizon projection screen."""
...
WORKER = Path("scripts/run_mujoco_v18_4_receding_joint_projection.py")
SIGNATURE_VERSION = "mujoco-v18-4-receding-joint-projection-v1"
DATA_LOCAL_NODE = "node003"
CPU_TRAINING_JUSTIFICATION = (
    "The v18.4 label-free development screen applies four deterministic "
    "NumPy causal receding-horizon projectors to 120 server-only node003 "
    "traces, then exact-audits only the selected candidate; paths are "
    "independent and numerical workers are single-threaded."
)
...
def build_scheduler_spec(args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "description": "Freq-HRL v18.4 causal receding projection screen",
...
def write_registration(args: argparse.Namespace) -> None:
    ...
    (output / "selection_registration.json").write_text(
        json.dumps({
            "status": "registered_before_v18_4_reused_path_access",
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v18.4 resources must be positive")
    ...
    if (
        ...
        raise SystemExit("v18.4 frozen core identity mismatch")
