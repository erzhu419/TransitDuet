#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14 behavior-development screen via scheduleurm."""
...
SIGNATURE_VERSION = "mujoco-v14-behavior-screen-v1"
...
def runtime_identity() -> dict[str, str]:
    ...
    if clean.returncode != 0:
        raise RuntimeError("v14 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14-Behavior-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14 screen {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        ...
        "checkpoint_selection_seeds": list(
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14 screen preregistration differs")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14 screen: {len(missing)} cells missing; "
            f"first={missing[0]}"
        )
    issues: list[str] = []
    cells = []
    for environment, arm, seed, path in expected:
        ...
        checks = {
            ...
            "responsibility": (
                summary.get("responsibility_mode")
            ...
            "source_verified": (
                summary.get("source_identity_status") == "verified"
            ),
        }
        issues.extend(
    ...
    if issues:
        raise SystemExit("v14 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14 development cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14 screen arm registry")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14 screen nodes: {unknown_nodes}")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14 screen frozen algorithm identity mismatch")
    ...
    if not cells:
        print("no v14 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14 behavior screen {args.run_name}"
    ))
