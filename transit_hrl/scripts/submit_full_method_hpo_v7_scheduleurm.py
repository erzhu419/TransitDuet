#!/usr/bin/env python3
"""Submit or merge support-only Freq-HRL v7 HPO through scheduleurm."""
...
HPO_SIGNATURE_VERSION = "full-hpo-v7-support-only"
SUBMIT_SCRIPT_PATH = Path(__file__).resolve()
CPU_JUSTIFICATION = (
    "Each v7 HPO cell trains one frozen checkpoint and is explicitly "
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        ...
        "description": (
            f"Freq-HRL v7 support HPO {variant_id} {candidate_id} "
...
def build_preflight_spec(args: argparse.Namespace, *, node: str) -> dict[str, object]:
    ...
    return {
        "project": str(args.project),
        "description": f"Freq-HRL v7 environment preflight on {node}",
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    print(f"merged {len(directories)} v7 HPO cells into {output}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--project", default="Freq-HRL-v7")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if unknown_variants or unknown_candidates or unknown_nodes:
        raise SystemExit(
            f"invalid v7 matrix: variants={unknown_variants}, "
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"cannot freeze v7 HPO source identity: {exc}") from exc
    if args.environment_preflight:
        execute_bulk(
            [build_preflight_spec(args, node=node) for node in args.nodes],
            dry_run=bool(args.dry_run),
            intent_label=f"Freq-HRL v7 environment preflight {args.run_name}",
        )
    ...
    if not cells:
        print("no v7 HPO cells require submission")
        return
    print(
    ...
    execute_bulk(
        ...
        dry_run=bool(args.dry_run),
        intent_label=f"Freq-HRL v7 support HPO {args.run_name}",
    )
    if args.dispatch and not args.dry_run:
        execute([sys.executable, str(SCHEDULER), "dispatch"], dry_run=False)
    print(
