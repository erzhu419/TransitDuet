#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v24 upper-target development."""
...
SIGNATURE_VERSION = "mujoco-v24-policy-mean-upper-projection-target-development-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def build_training_command(
    ...
    control_args = [
        ...
        *(str(seed) for seed in spec.EVALUATION_SEEDS),
        ...
        "--upper-projection-target-aggregation",
        str(arm_spec["upper_projection_target_aggregation"]),
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": f"Freq-HRL v24 {environment} {arm} {optimizer_seed}",
        ...
        "cpu_training_justification": (
            "Independent v24 MuJoCo cells are CPU-bound and single-threaded."
        ),
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v24 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v24 task is not done: {task.get('id')} status={task.get('status')}"
            )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v24 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v24 small-result cells")
...
def _verify_frozen_core() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("v24 frozen freq_hrl source differs from its revision")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v24 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v24 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v24 environment subset")
    if (
...
def main() -> None:
    ...
    execute_bulk(
        ...
        intent_label=f"Freq-HRL {spec.DEVELOPMENT_PROTOCOL_VERSION} {args.run_name}",
    )
