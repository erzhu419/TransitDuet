#!/usr/bin/env python3
"""Analyze the frozen v14.26 robust paired-FD preflight."""
...
def _validate_payload(payload: dict[str, Any], environment: str, seed: int) -> None:
    ...
    if mismatches:
        raise ValueError(f"v14.26 payload contract mismatch: {mismatches}")
    if int(payload.get("actor_update_parameter_count", 0)) < 1:
        raise ValueError("v14.26 payload lacks output-bias parameters")
    ...
    if not isinstance(metrics, dict) or set(metrics) != {"upper", "lower"}:
        raise ValueError("v14.26 payload lacks both critic metric blocks")
    ...
    for level in ("upper", "lower"):
        ...
        if not isinstance(directions, dict) or level not in directions:
            raise ValueError("v14.26 payload lacks paired-direction metrics")
        ...
        if not isinstance(train, dict) or not isinstance(holdout, dict):
            raise ValueError("v14.26 payload lacks paired path diagnostics")
        if train.get("path_count") != spec.EXPECTED_PAIRED_TRAIN_DIRECTION_COUNT:
            raise ValueError("v14.26 paired train direction count drifted")
        if holdout.get("path_count") != spec.EXPECTED_PAIRED_HOLDOUT_DIRECTION_COUNT:
            raise ValueError("v14.26 paired holdout direction count drifted")
        if not isinstance(mode_cosines, dict) or len(mode_cosines) != 4:
            raise ValueError("v14.26 lacks four disturbance-mode cosines")
        ...
        if not math.isclose(
            ...
            raise ValueError("v14.26 minimum mode cosine is inconsistent")
        gate = gate and (
...
def analyze_run(run_name: str, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    for environment in spec.ENVIRONMENTS:
        for seed in spec.OPTIMIZER_SEEDS:
            ...
            if not path.is_file():
                raise FileNotFoundError(f"missing v14.26 probe result: {path}")
            ...
            rows.append({
                ...
                "upper_action_permutation_mse_increase": float(
                    payload["critic_metrics"]["upper"]
                    ["action_permutation_mse_increase_fraction"]
                ),
                "lower_action_permutation_mse_increase": float(
                    payload["critic_metrics"]["lower"]
                    ["action_permutation_mse_increase_fraction"]
                ),
                "upper_paired_holdout_cosine": (
                    None if "upper" not in direction
                    else float(direction["upper"]["holdout_direction_cosine"])
                ),
                "lower_paired_holdout_cosine": (
                    None if "lower" not in direction
                    else float(direction["lower"]["holdout_direction_cosine"])
                ),
    ...
    result = {
        "analysis_version": "mujoco_v14_26_robust_paired_fd_preflight_v1",
        "evidence_role": spec.EVIDENCE_ROLE,
        "selection_contract": spec.SELECTION_CONTRACT,
        "cell_count": len(rows),
        "supported_cell_count": supported,
        "status": (
            "robust_paired_fd_preflight_supported"
            if supported == spec.EXPECTED_CELL_COUNT
            else "robust_paired_fd_preflight_not_supported"
        ),
        "cells": rows,
    }
    target = output_dir or (ROOT / "results" / run_name / "analysis")
    target.mkdir(parents=True, exist_ok=True)
    (target / "robust_paired_fd_preflight.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    with (target / "robust_paired_fd_preflight.csv").open(
