#!/usr/bin/env python3
"""Submit Stage-8B counterfactual plan-value qualification."""
...
from scripts import pointmaze_plan_validity_stage8b_spec as spec  # noqa: E402
...
CPU_JUSTIFICATION = (
    "Independent PointMaze PPO roots and paired deterministic branch replay "
    "are single-threaded CPU workloads dynamically placed on the Linux pool."
)
...
def task_specification(
    ...
    *,
    ...
    return {
        ...
        "description": (
            f"Freq-HRL PointMaze stage8b {phase} {policy} "
...
def sync_results(
    ...
    *,
    ...
    for policy, optimizer_seed in spec.cells(preflight=preflight):
        ...
        if task is None:
            raise SystemExit(f"Stage-8B sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"Stage-8B task is not done: {task.get('id')} "
    ...
    if pending:
        ...
        raise SystemExit(
            f"Stage-8B result sync incomplete: {len(pending)} cells; "
    ...
    for signature, path, _ in expected:
        ...
        if (
            ...
            raise SystemExit(f"invalid synced Stage-8B result: {signature}")
    ...
    print(f"synced {len(expected)} Stage-8B result JSON files")
...
def main() -> int:
    ...
    registration = {
        ...
        "algorithm_revision": spec.ALGORITHM_REVISION,
        "evidence_stage": (
            "preflight" if args.preflight else spec.EVIDENCE_STAGE
        ),
