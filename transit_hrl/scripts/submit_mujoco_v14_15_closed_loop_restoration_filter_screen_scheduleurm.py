#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.15 closed-loop restoration-filter screen."""
...
SIGNATURE_VERSION = "mujoco-v14-15-closed-loop-restoration-filter-screen-v1"
...
def _rank_not_worse(
    ...
    *,
    ...
    left = tuple(map(float, candidate))
    right = tuple(map(float, baseline))
    if not left or len(left) != len(right):
        return False
    for candidate_value, baseline_value in zip(left, right, strict=True):
...
def _closed_loop_guard_contract_valid(
    ...
    *,
    ...
    if not expected:
        return bool(
            summary.get(f"{prefix}enabled") is False
            and int(summary.get(f"{prefix}path_count", -1)) == 0
            and summary.get(f"{prefix}contract") == "disabled"
            and summary.get(
                "deployment_frequency_closed_loop_guard_seed_roots"
            ) == []
            and all(
                float(row.get(f"{prefix}enabled", -1.0)) == 0.0
                for row in history
            )
            and summary.get(
    ...
    initial_worst = float(initial.get(
        f"{prefix}worst_frequency_violation_after", float("nan")
    ))
    funnel_limit = initial_worst * float(restoration_funnel_multiplier)
    if not (
        summary.get(f"{prefix}enabled") is True
        and summary.get(f"{prefix}contract") == contract
        and int(summary.get(f"{prefix}path_count", -1)) == expected_paths
        and int(summary.get(f"{prefix}constraint_count", -1))
        == expected_constraints
        and summary.get(
            "deployment_frequency_closed_loop_guard_seed_roots"
        ) == list(spec.DEPLOYMENT_FREQUENCY_CLOSED_LOOP_GUARD_SEEDS)
        and int(summary.get(f"{prefix}evaluation_count", -1))
        ...
        and int(summary.get(
            ...
            f"{prefix}initial_frequency_violation_count", -2
        ))
        and summary.get(
            "deployment_frequency_closed_loop_restoration_filter_enabled"
        ) is bool(restoration_expected)
        and math.isclose(
            float(summary.get(
                "deployment_frequency_closed_loop_restoration_min_reduction",
                float("nan"),
            )),
            float(restoration_min_reduction),
            rel_tol=0.0,
            abs_tol=1e-15,
        )
        and math.isclose(
            ...
            float(restoration_funnel_multiplier),
            rel_tol=0.0,
            abs_tol=1e-15,
        )
        and int(summary.get(
            ...
            float(row.get(f"{prefix}accepted", 0.0))
            for row in history
        )))
        and int(summary.get(f"{prefix}evaluation_count", -1))
        == int(round(sum(
            float(row.get(f"{prefix}evaluation_count", 0.0))
            for row in history
        ))) + 1
    ):
        return False
    if restoration_expected and not (
        math.isfinite(initial_merit)
        and initial_merit >= 0.0
        and math.isfinite(initial_worst)
        and initial_worst >= 0.0
        and math.isclose(
            float(summary.get(
        ...
        and math.isclose(
            float(summary.get(
                f"{prefix}initial_frequency_violation_merit", float("nan")
            )),
            initial_merit,
            rel_tol=0.0,
            abs_tol=1e-12,
        ...
        and math.isclose(
            float(summary.get(
                f"{prefix}initial_worst_frequency_violation", float("nan")
            )),
            initial_worst,
            rel_tol=0.0,
            abs_tol=1e-12,
        ...
        return False
    baseline = dict(summary.get(
        "deployment_frequency_closed_loop_guard_baseline"
    ) or {})
    assignments = dict(summary.get(
    ...
    if not (
        ...
        and baseline.get("checkpoint_selection_rows_used") == 0
        and baseline.get("heldout_rows_used") == 0
        and len(str(baseline.get("parameter_sha256", ""))) == 64
        and len(assignments) == expected_paths
        and all(
            list(assignments.values()).count(mode)
            == len(spec.DEPLOYMENT_FREQUENCY_CLOSED_LOOP_GUARD_SEEDS)
            for mode in spec.TRAINING_DISTURBANCE_MODES
        )
    ):
        return False
    previous_rank = list(initial.get(f"{prefix}rank_after", []))
    previous_frequency_violations = int(round(float(initial.get(
        f"{prefix}frequency_violation_count", -1.0
    ))))
    if not previous_rank or float(initial.get(
        f"{prefix}reward_violation_count", -1.0
    )) > 0.0:
        return False
    previous_merit = initial_merit
    previous_worst = initial_worst
    for row in history[1:]:
        rank_before = list(row.get(f"{prefix}rank_before", []))
        rank_after = list(row.get(f"{prefix}rank_after", []))
        frequency_violations = int(round(float(row.get(
            f"{prefix}frequency_violation_count", -1.0
        ))))
        common_valid = bool(
            float(row.get(f"{prefix}enabled", 0.0)) == 1.0
            and str(row.get(f"{prefix}contract", "")) == contract
            and int(round(float(row.get(f"{prefix}path_count", -1.0))))
            == expected_paths
            and int(round(float(row.get(
                f"{prefix}constraint_count", -1.0
            )))) == expected_constraints
            and np.allclose(rank_before, previous_rank, atol=1e-10, rtol=0.0)
            and float(row.get(f"{prefix}reward_violation_count", -1.0))
            <= spec.MAXIMUM_CLOSED_LOOP_REWARD_VIOLATIONS
            and 0.0 <= float(row.get(f"{prefix}step_fraction", -1.0)) <= 1.0
        )
        if not common_valid:
            return False
        if not restoration_expected:
            if not (
                _rank_not_worse(rank_after, rank_before)
                and frequency_violations <= previous_frequency_violations
            ):
                return False
            previous_rank = rank_after
            previous_frequency_violations = frequency_violations
            continue

        merit_before = float(row.get(
            f"{prefix}restoration_merit_before", float("nan")
        ))
        merit_after = float(row.get(
            f"{prefix}restoration_merit_after", float("nan")
        ))
        worst_before = float(row.get(
            f"{prefix}worst_frequency_violation_before", float("nan")
        ))
        worst_after = float(row.get(
            f"{prefix}worst_frequency_violation_after", float("nan")
        ))
        trial_trace = row.get(f"{prefix}trial_trace")
        evaluation_count = int(round(float(row.get(
            f"{prefix}evaluation_count", -1.0
        ))))
        if not (
            float(row.get(f"{prefix}restoration_filter_enabled", 0.0)) == 1.0
            and math.isclose(merit_before, previous_merit, abs_tol=1e-12)
            and math.isclose(worst_before, previous_worst, abs_tol=1e-12)
            and isinstance(trial_trace, list)
            and len(trial_trace) == evaluation_count
            and len(trial_trace) >= 1
            and math.isclose(
                float(trial_trace[0].get("fraction", float("nan"))),
                1.0,
                abs_tol=1e-15,
            )
            and math.isclose(
                float(row.get(
                    f"{prefix}restoration_funnel_limit", float("nan")
                )),
                funnel_limit,
                abs_tol=1e-12,
            )
            and str(row.get(f"{prefix}restoration_phase_before"))
            == ("restoration" if previous_frequency_violations > 0 else "maintenance")
            and str(row.get(f"{prefix}restoration_phase_after"))
            == ("restoration" if frequency_violations > 0 else "maintenance")
        ):
            return False
        accepted_indices: list[int] = []
        previous_fraction = float("inf")
        for index, trial in enumerate(trial_trace):
            if not isinstance(trial, dict):
                return False
            fraction = float(trial.get("fraction", float("nan")))
            trial_reward = int(trial.get("reward_violation_count", -1))
            trial_frequency = int(trial.get("frequency_violation_count", -1))
            trial_merit = float(trial.get(
                "frequency_violation_merit", float("nan")
            ))
            trial_worst = float(trial.get(
                "worst_frequency_violation", float("nan")
            ))
            if not (
                math.isfinite(fraction)
                and 0.0 <= fraction <= 1.0
                and fraction < previous_fraction
                and math.isfinite(trial_merit)
                and trial_merit >= 0.0
                and math.isfinite(trial_worst)
                and trial_worst >= 0.0
            ):
                return False
            expected_reasons: list[str] = []
            if trial_reward != 0:
                expected_reasons.append("reward_floor_violation")
            if previous_frequency_violations == 0:
                if trial_frequency != 0:
                    expected_reasons.append("maintenance_frequency_violation")
            else:
                required_merit = previous_merit * (
                    1.0 - float(restoration_min_reduction) * fraction
                )
                if trial_merit > required_merit + 1e-12:
                    expected_reasons.append("restoration_merit_not_reduced")
                if trial_worst > funnel_limit + 1e-12:
                    expected_reasons.append("restoration_funnel_exceeded")
            trial_accepted = bool(trial.get("accepted"))
            if (
                trial_accepted != (not expected_reasons)
                or list(trial.get("rejection_reasons", [])) != expected_reasons
            ):
                return False
            if trial_accepted:
                accepted_indices.append(index)
            previous_fraction = fraction
        if accepted_indices != [len(trial_trace) - 1]:
            return False
        selected_trial = trial_trace[-1]
        selected_fraction = float(selected_trial["fraction"])
        effective_update = bool(float(row.get(f"{prefix}accepted", 0.0)))
        if not (
            math.isclose(
                selected_fraction,
                float(row.get(f"{prefix}step_fraction", float("nan"))),
                abs_tol=1e-15,
            )
            and effective_update == (
                selected_fraction > 0.0
                and float(row.get(f"{prefix}final_actor_rms", 0.0)) > 1e-12
            )
            and math.isclose(
                float(selected_trial["frequency_violation_merit"]),
                merit_after,
                abs_tol=1e-12,
            )
            and math.isclose(
                float(selected_trial["worst_frequency_violation"]),
                worst_after,
                abs_tol=1e-12,
            )
            and int(selected_trial["frequency_violation_count"])
            == frequency_violations
            and np.allclose(
                selected_trial.get("rank", []), rank_after,
                atol=1e-10, rtol=0.0,
            )
        ):
            return False
        full_trial = trial_trace[0]
        if not (
            math.isclose(
                float(full_trial["frequency_violation_merit"]),
                float(row.get(
                    f"{prefix}restoration_merit_full_step", float("nan")
                )),
                abs_tol=1e-12,
            )
            and math.isclose(
                float(full_trial["worst_frequency_violation"]),
                float(row.get(
                    f"{prefix}worst_frequency_violation_full_step",
                    float("nan"),
                )),
                abs_tol=1e-12,
            )
        ...
        previous_rank = rank_after
        previous_frequency_violations = frequency_violations
        previous_merit = merit_after
        previous_worst = worst_after
    selected_frequency_violations = int(summary.get(
        f"{prefix}selected_frequency_violation_count", -1
    ))
    selected_merit = float(summary.get(
        f"{prefix}selected_frequency_violation_merit", float("nan")
    ))
    selected_worst = float(summary.get(
        f"{prefix}selected_worst_frequency_violation", float("nan")
    ))
    restoration_summary_valid = True
    if restoration_expected:
        restoration_summary_valid = bool(
            math.isclose(
                float(summary.get(
                    f"{prefix}training_final_frequency_violation_merit",
                    float("nan"),
                )),
                previous_merit,
                abs_tol=1e-12,
            ...
            and math.isclose(
                float(summary.get(
                    f"{prefix}training_final_worst_frequency_violation",
                    float("nan"),
                )),
                previous_worst,
                abs_tol=1e-12,
            )
            and math.isfinite(selected_merit)
            and selected_merit <= initial_merit + 1e-12
            and math.isfinite(selected_worst)
            and selected_worst <= funnel_limit + 1e-12
            and (
                selected_frequency_violations > 0
                or selected_worst <= 1e-10
            )
        )
    return bool(
        np.allclose(
            summary.get(f"{prefix}training_final_rank", []),
            previous_rank,
            atol=1e-10,
            rtol=0.0,
        )
        and (
            ...
            or _rank_not_worse(
                summary.get(f"{prefix}selected_rank", []),
                summary.get(f"{prefix}initial_rank", []),
            )
        )
        and restoration_summary_valid
    )
...
def build_training_command(
    ...
    *,
    ...
    if bool(arm_spec["deployment_frequency_closed_loop_trust_region"]):
        command.extend([
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        ...
        "groupwise_arms": list(spec.GROUPWISE_ARMS),
        "replay_arms": list(spec.REPLAY_ARMS),
        "trust_arms": list(spec.TRUST_ARMS),
        "joint_arms": list(spec.JOINT_ARMS),
        "closed_loop_arms": list(spec.CLOSED_LOOP_ARMS),
        "restoration_arms": list(spec.RESTORATION_ARMS),
        "authorizing_arms": list(spec.AUTHORIZING_ARMS),
        ...
        "deployment_frequency_closed_loop_guard_seeds": list(
            spec.DEPLOYMENT_FREQUENCY_CLOSED_LOOP_GUARD_SEEDS
        ),
        ...
        "development_selection_gates": {
            ...
            "minimum_groupwise_group_count": (
                spec.MINIMUM_GROUPWISE_GROUP_COUNT
            ),
            "minimum_replay_group_count": spec.MINIMUM_REPLAY_GROUP_COUNT,
            "expected_reward_guard_group_count": (
                spec.EXPECTED_REWARD_GUARD_GROUP_COUNT
            ),
            "expected_anchor_replay_path_count": (
            ...
            "minimum_closed_loop_guard_evaluations": (
                spec.MINIMUM_CLOSED_LOOP_GUARD_EVALUATIONS
            ),
            "minimum_closed_loop_effective_updates": (
                spec.MINIMUM_CLOSED_LOOP_EFFECTIVE_UPDATES
            ),
            "maximum_closed_loop_reward_violations": (
                spec.MAXIMUM_CLOSED_LOOP_REWARD_VIOLATIONS
            ),
            "maximum_closed_loop_frequency_violations": (
            ...
            "restoration_trial_trace_required": True,
            "restoration_merit_monotonicity_required": True,
            "strict_feasibility_maintenance_required": True,
            "minimum_trust_accepted_steps": (
                spec.MINIMUM_TRUST_ACCEPTED_STEPS
            ),
            "minimum_groupwise_accepted_steps": (
                spec.MINIMUM_GROUPWISE_ACCEPTED_STEPS
            ),
            "minimum_groupwise_target_reached_groups": (
            ...
            "maximum_group_reward_budget_violations": (
        ...
        if phase == "anchor":
            ...
            checks = {
                **common_checks,
                ...
                "deployment_ppo_trust_region": bool(summary.get(
                    "deployment_frequency_ppo_trust_region", False
                )) == bool(arm_spec[
                    "deployment_frequency_ppo_trust_region"
                ]),
                "deployment_ppo_trust_backtracks": int(summary.get(
                    "deployment_frequency_ppo_trust_region_backtracks", -1
                )) == int(arm_spec[
                    "deployment_frequency_ppo_trust_region_backtracks"
                ]),
                "deployment_closed_loop_trust_region": bool(summary.get(
                    "deployment_frequency_closed_loop_trust_region", False
                )) == bool(arm_spec[
                    "deployment_frequency_closed_loop_trust_region"
                ]),
                "deployment_closed_loop_trust_backtracks": int(summary.get(
                    "deployment_frequency_closed_loop_trust_region_backtracks",
                    -1,
                )) == int(arm_spec[
                    "deployment_frequency_closed_loop_trust_region_backtracks"
                ]),
                "deployment_closed_loop_restoration_filter": bool(summary.get(
                    "deployment_frequency_closed_loop_restoration_filter_enabled",
                    False,
                )) == bool(arm_spec[
                    "deployment_frequency_closed_loop_restoration_filter"
                ]),
                "deployment_closed_loop_restoration_min_reduction": math.isclose(
                    float(summary.get(
                        "deployment_frequency_closed_loop_restoration_min_reduction",
                        float("nan"),
                    )),
                ...
                "deployment_closed_loop_restoration_funnel_multiplier": math.isclose(
                    float(summary.get(
                        "deployment_frequency_closed_loop_restoration_funnel_multiplier",
                        float("nan"),
                    )),
                ...
                "closed_loop_guard_contract": (
                    _closed_loop_guard_contract_valid(
                        summary,
                        history,
                        expected=bool(arm_spec[
                            "deployment_frequency_closed_loop_trust_region"
                        ]),
                        restoration_expected=bool(arm_spec[
                            "deployment_frequency_closed_loop_restoration_filter"
                        ]),
                        restoration_min_reduction=float(arm_spec[
                            "deployment_frequency_closed_loop_restoration_min_reduction"
                        ]),
                ...
                "closed_loop_guard_anchor_hash": (
                    dict(summary.get(
                        "deployment_frequency_closed_loop_guard_baseline"
                    ) or {}).get("parameter_sha256")
                    == anchor["summary"].get("frozen_parameter_sha256")
                ) if bool(arm_spec[
                    "deployment_frequency_closed_loop_trust_region"
                ]) else True,
                "anchor_replay_contract": (
                    bool(summary.get(
