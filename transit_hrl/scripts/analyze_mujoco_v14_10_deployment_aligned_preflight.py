#!/usr/bin/env python3
"""Audit the scoped single-seed MuJoCo v14.10 preflight."""
...
from scripts.analyze_mujoco_v14_10_deployment_aligned_screen import (  # noqa: E402
    MAX_METRICS,
    METRICS,
    TRACE_KEYS,
...
ANALYSIS_VERSION = "mujoco_v14_10_deployment_aligned_preflight_analysis_v1"
FREQUENCY_METRICS = (
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _load_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise ValueError(f"invalid v14.10 preflight cell: {path}")
...
def _mode_payload(rows: list[dict[str, str]], mode: str) -> dict[str, Any]:
    ...
    if len(selected) != len(spec.DEVELOPMENT_EVALUATION_SEEDS):
        raise ValueError(f"v14.10 preflight mode is incomplete: {mode}")
    payload = {
        metric: (
...
def _projection_diagnostics(
    ...
    for level in ("upper", "lower"):
        ...
        by_level[level] = {
            "active": active,
            "guard_attempted_count": len(attempted),
            "guard_accepted_count": len(accepted),
            "accepted_power_reduction_mean": float(
                np.mean(reductions) if reductions else 0.0
            ),
            "pass": passed,
        }
    return {"pass": bool(all(passes)), "by_level": by_level}
...
def analyze_preflight(
    ...
    if environment not in spec.ENVIRONMENTS:
        raise ValueError(f"invalid v14.10 preflight environment: {environment}")
    if int(optimizer_seed) not in spec.OPTIMIZER_SEEDS:
        raise ValueError("invalid v14.10 preflight optimizer seed")
    ...
    if not identity_pass:
        raise ValueError("v14.10 preflight scope or frozen identity mismatch")
    ...
    if not (
        ...
        raise ValueError("v14.10 preflight anchor identity mismatch")
    ...
    for arm in spec.ARMS:
        ...
        if not (
            ...
            raise ValueError(f"v14.10 preflight cell identity mismatch: {arm}")
        ...
        if not (
            ...
            raise ValueError(f"v14.10 preflight anchor provenance mismatch: {arm}")
        if (
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.10 preflight paired baseline mismatch: {arm}"
                )
        cells[arm] = cell
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    lines = [
        "# MuJoCo v14.10 Deployment-Aligned Preflight",
        "",
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_10_preflight status={decision['status']} "
