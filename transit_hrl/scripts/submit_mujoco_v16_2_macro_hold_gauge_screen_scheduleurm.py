#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v16.2 gauge screen."""
...
SIGNATURE_VERSION = "mujoco-v16-2-macro-hold-gauge-screen-v1"
...
def build_training_command(
    ...
    return (
        " ".join([*environment_variables, shlex.join(command)])
        + " && echo DONE"
    )
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": (
            f"Freq-HRL v16.2 {environment} {arm} {optimizer_seed}"
        ),
        ...
        "cpu_training_justification": (
            "Independent MuJoCo routing cells are CPU-bound and single-threaded."
        ),
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v16.2 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v16.2 task is not done: {task.get('id')} "
                f"status={task.get('status')}"
            )
        path = ROOT / cell_relative_dir(
            args.run_name, environment, arm, seed
        )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v16.2 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v16.2 cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v16.2 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v16.2 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v16.2 environment subset")
    if (
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v16.2 frozen algorithm identity mismatch")
    if args.sync_only:
        sync_results(args)
        return
