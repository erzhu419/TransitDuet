#!/usr/bin/env python3
"""Submit the frozen v18.5 actor-floor signal diagnostic."""
...
WORKER = Path("scripts/run_mujoco_v18_5_actor_floor_signal.py")
SIGNATURE_VERSION = "mujoco-v18-5-actor-floor-signal-v1"
DATA_LOCAL_NODE = "node003"
CPU_TRAINING_JUSTIFICATION = (
    "The v18.5 diagnostic computes two deterministic causal responsibility "
    "signals on 120 server-only node003 traces; paths are independent, no "
    "target or checkpoint is read, and numerical workers are single-threaded."
)
...
def build_scheduler_spec(args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "description": "Freq-HRL v18.5 causal actor-floor signal diagnostic",
        "cmd": build_command(args),
        "cwd": str(ROOT),
        "signature": f"Freq-HRL/{SIGNATURE_VERSION}/{args.run_name}/signal",
        "resource_family": f"Freq-HRL/{SIGNATURE_VERSION}/signal",
        "vram": 0,
...
def write_registration(args: argparse.Namespace) -> None:
    ...
    (output / "signal_registration.json").write_text(
        json.dumps({
            "status": "registered_before_v18_5_reused_path_access",
            ...
            "scheduler_contract": {
                ...
                "require_node": DATA_LOCAL_NODE,
                "binding_reason": (
                    "v17.8 traces remain server-only on node003"
                ),
                "cpu": int(args.cpu),
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v18.5 resources must be positive")
    ...
    if (
        ...
        raise SystemExit("v18.5 frozen core identity mismatch")
