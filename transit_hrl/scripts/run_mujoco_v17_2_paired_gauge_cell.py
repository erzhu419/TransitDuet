#!/usr/bin/env python3
"""Train one v17.2 policy and evaluate a paired frozen-gauge intervention."""
...
def paired_intervention_audit(
    ...
    if len(control) != len(control_rows) or len(candidate) != len(candidate_rows):
        raise ValueError("v17.2 paired rows contain duplicate paths")
    if set(control) != set(candidate):
        raise ValueError("v17.2 interventions do not share heldout paths")
    trace_mismatches = {
        key: sum(
    ...
    *,
    ...
    for control in control_rows:
        _, row = rollout_hierarchical(
            ...
            lower_action_router_alpha=float(alpha),
...
def run_cell(
    *,
    ...
    if str(alpha_arm) not in spec.ALPHA_ARMS:
        raise ValueError(f"unknown v17.2 alpha arm: {alpha_arm}")
    ...
    if parameter_sha256 != _model_parameter_sha256(model):
        raise RuntimeError("v17.2 paired intervention mutated the checkpoint")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--alpha-arm", choices=spec.ALPHA_ARMS, required=True)
...
def main() -> None:
    ...
    if int(args.optimizer_seed) not in spec.OPTIMIZER_SEEDS:
        raise SystemExit("optimizer seed is outside the frozen v17.2 design")
    if _complete(args.full_output_dir):
        print(f"v17.2 reuse=complete_server_cell source={args.full_output_dir}")
    else:
        payload, rows, model = run_cell(
            env_id=args.env_id,
            alpha_arm=args.alpha_arm,
    ...
    print(f"DONE mujoco_v17_2_paired_gauge output={args.export_output_dir}")
