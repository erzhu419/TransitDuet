#!/usr/bin/env python3
"""Analyze the frozen v14.23 frequency-horizon critic preflight."""
...
def _validate_payload(payload: dict[str, Any], environment: str, seed: int) -> None:
    ...
    if mismatches:
        raise ValueError(f"v14.23 payload contract mismatch: {mismatches}")
    ...
    if not isinstance(metrics, dict) or set(metrics) != {"upper", "lower"}:
        raise ValueError("v14.23 payload lacks both critic metric blocks")
    ...
    for level in ("upper", "lower"):
        ...
        if payload.get("gradient_error") is None:
            if not isinstance(gradients, dict) or level not in gradients:
                raise ValueError("v14.23 payload lacks actor-gradient metrics")
            gate = gate and (
...
def analyze_run(run_name: str, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    for environment in spec.ENVIRONMENTS:
        for seed in spec.OPTIMIZER_SEEDS:
            ...
            if not path.is_file():
                raise FileNotFoundError(f"missing v14.23 probe result: {path}")
    ...
    result = {
        "analysis_version": "mujoco_v14_23_frequency_horizon_critic_preflight_v1",
        "evidence_role": spec.EVIDENCE_ROLE,
        "selection_contract": spec.SELECTION_CONTRACT,
        "cell_count": len(rows),
        "supported_cell_count": supported,
        "status": (
            "frequency_horizon_critic_preflight_supported"
            if supported == spec.EXPECTED_CELL_COUNT
            else "frequency_horizon_critic_preflight_not_supported"
        ),
        "cells": rows,
    }
    target = output_dir or (ROOT / "results" / run_name / "analysis")
    target.mkdir(parents=True, exist_ok=True)
    (target / "frequency_horizon_critic_preflight.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    with (target / "frequency_horizon_critic_preflight.csv").open(
