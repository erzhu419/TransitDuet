#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v23 upper-target development."""
...
SIGNATURE_VERSION = "mujoco-v23-causal-upper-projection-target-development-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": f"Freq-HRL v23 {environment} {arm} {optimizer_seed}",
        ...
        "cpu_training_justification": (
            "Independent v23 MuJoCo cells are CPU-bound and single-threaded."
        ),
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v23 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v23 task is not done: {task.get('id')} status={task.get('status')}"
            )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v23 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v23 small-result cells")
...
def _verify_frozen_core() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("v23 frozen freq_hrl source differs from its revision")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v23 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v23 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v23 environment subset")
    if (
