#!/usr/bin/env python3
"""Train one v17.4 policy and evaluate the paired gauge intervention."""
...
def paired_intervention_audit(
    ...
    if len(control) != len(control_rows) or len(candidate) != len(candidate_rows):
        raise ValueError("v17.4 paired rows contain duplicate paths")
    if set(control) != set(candidate):
        raise ValueError("v17.4 interventions do not share heldout paths")
    trace_mismatches = {
        key: sum(
...
def run_cell(
    *,
    ...
    if parameter_sha256 != _model_parameter_sha256(model):
        raise RuntimeError("v17.4 paired intervention mutated the checkpoint")
...
def main() -> None:
    ...
    if _complete(args.full_output_dir):
        print(f"v17.4 reuse=complete_server_cell source={args.full_output_dir}")
    else:
    ...
    print(f"DONE mujoco_v17_4_paired_gauge output={args.export_output_dir}")
