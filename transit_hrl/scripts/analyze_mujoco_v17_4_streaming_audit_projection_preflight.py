#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v17.4 paired streaming audit preflight."""
...
from scripts import (  # noqa: E402
    mujoco_v17_4_streaming_audit_projection_preflight_spec as spec,
)
from scripts.run_mujoco_v17_4_paired_gauge_cell import (  # noqa: E402
    paired_intervention_audit,
)
from scripts.submit_mujoco_v17_4_streaming_audit_projection_preflight_scheduleurm import (  # noqa: E402
    cell_relative_dir,
)
...
def _validate_preregistration(root: Path, run_name: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("v17.4 preregistration or result-sync contract mismatch")
    return sync
...
def _split_interventions(
    ...
    if (
        ...
        raise ValueError("v17.4 paired intervention row counts are incomplete")
...
def _validate_cell(
    ...
    if (
        ...
        raise ValueError(
            f"v17.4 cell contract mismatch: {environment}/{optimizer_seed}"
        )
    ...
    if (
        ...
        raise ValueError(f"v17.4 heldout path registry mismatch: {environment}")
    if any(
        ...
        raise ValueError("v17.4 paired intervention strength mismatch")
    audit = paired_intervention_audit(control, candidate)
    if audit != summary.get("paired_intervention_audit"):
        raise ValueError("v17.4 stored paired audit does not reproduce")
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        ...
        "mean_upper_budget_feasible_rate": _mean(
            rows, "LowerRouterUpperBudgetFeasibleRate"
        ),
        "mean_lower_budget_satisfied_rate": _mean(
            rows, "LowerRouterLowerBudgetSatisfiedRate"
        ),
        "max_upper_budget_violation_rms": max(
            float(row["LowerRouterUpperBudgetViolationRMS"])
            for row in rows
        ),
        "max_lower_budget_violation_rms": max(
            float(row["LowerRouterLowerBudgetViolationRMS"])
            for row in rows
        ),
...
def analyze(run_name: str, *, root: Path = ROOT) -> dict[str, Any]:
    ...
    for environment in spec.ENVIRONMENTS:
        for optimizer_seed in spec.OPTIMIZER_SEEDS:
            ...
            gates = {
                ...
                "upper_hf_absolute_budget": (
                    candidate["upper_hf_power"]
                    <= spec.UPPER_HF_RMS_BUDGET ** 2
                    + spec.POWER_BUDGET_TOLERANCE
                ),
                "lower_lf_absolute_budget": (
                    candidate["lower_lf_power"]
                    <= spec.LOWER_LF_RMS_BUDGET ** 2
                    + spec.POWER_BUDGET_TOLERANCE
                ),
                "upper_budget_feasibility": (
                    candidate["mean_upper_budget_feasible_rate"]
                    >= spec.MINIMUM_UPPER_BUDGET_FEASIBLE_RATE
                    and candidate["max_upper_budget_violation_rms"]
                    <= spec.MAXIMUM_UPPER_BUDGET_VIOLATION_RMS
                ),
                "lower_lf_reduction": (
                    lower_reduction
    ...
    return {
        ...
        "cells": cells,
        "eligible_for_streaming_projection_multiseed": eligible,
        ...
        "analysis_contract": (
            ...
            "recomputed_directly_absolute_band_budgets_and_feasibility_"
            "legacy_protocol_flag_diagnostic_only_v2"
        ),
    }
...
def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    flattened = [{
        ...
        "control_upper_hf_power": row["control_summary"]["upper_hf_power"],
        "candidate_upper_hf_power": row["candidate_summary"][
            "upper_hf_power"
        ],
        "control_lower_lf_power": row["control_summary"]["lower_lf_power"],
        "candidate_lower_lf_power": row["candidate_summary"][
            "lower_lf_power"
        ],
        "candidate_upper_budget_feasible_rate": row["candidate_summary"][
            "mean_upper_budget_feasible_rate"
        ],
        "candidate_max_upper_budget_violation_rms": row[
            "candidate_summary"
        ]["max_upper_budget_violation_rms"],
...
def main() -> None:
    ...
    (output / "streaming_audit_projection_preflight.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    _write_csv(output / "streaming_audit_projection_cells.csv", result["cells"])
    print(json.dumps({
        "status": result["status"],
        "eligible_for_streaming_projection_multiseed": result[
            "eligible_for_streaming_projection_multiseed"
        ],
