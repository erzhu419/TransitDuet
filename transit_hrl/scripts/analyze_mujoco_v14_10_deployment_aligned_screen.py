#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v14.10 deployment-aligned screen."""
...
ANALYSIS_VERSION = "mujoco_v14_10_deployment_aligned_screen_analysis_v1"
METRICS = (
    "episode_return",
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.10 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    ...
    if len(set(keys)) != len(keys) or set(keys) != expected_keys:
        raise ValueError(f"v14.10 evaluation path registry mismatch: {path}")
    return rows
...
def _finite_mean(rows: list[dict[str, str]], metric: str) -> float:
    ...
    if values.size == 0 or not np.all(np.isfinite(values)):
        raise ValueError(f"non-finite v14.10 metric: {metric}")
...
def _bounds(
    ...
    *,
    ...
    if array.size < 2 or not np.all(np.isfinite(array)):
        raise ValueError("v14.10 bootstrap requires finite optimizer replicates")
...
def _actor_vectors(path: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not isinstance(model, dict):
        raise ValueError(f"invalid v14.10 checkpoint payload: {path}")
    vectors: list[np.ndarray] = []
    for actor_name in ("upper_actor", "lower_actor"):
        actor = model.get(actor_name)
        if not isinstance(actor, dict) or not actor:
            raise ValueError(f"missing v14.10 checkpoint actor {actor_name}: {path}")
        values = []
        for name in sorted(actor):
            tensor = actor[name]
            if not torch.is_tensor(tensor):
                raise ValueError(
                    f"non-tensor v14.10 actor parameter {actor_name}.{name}"
                )
            values.append(
...
def _actor_rms_difference(candidate: Path, comparator: Path) -> dict[str, float]:
    ...
    if (
        ...
        raise ValueError("v14.10 paired actor checkpoint shapes differ")
...
def load_replicates(
    ...
    if (
        ...
        raise ValueError("v14.10 preregistration identity mismatch")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("v14.10 analysis requires a complete merged manifest")
        ...
        for arm, arm_spec in spec.ARMS.items():
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not np.allclose(
                    ...
                    raise ValueError(
                        f"v14.10 router strength mismatch: {environment}/{arm}/{seed}"
                    )
                for mode in spec.EVALUATION_DISTURBANCE_MODES:
                    mode_rows = [
                    ...
                    for trace in TRACE_KEYS:
                        ...
                        if any(len(value) != 64 for value in values):
                            raise ValueError(f"invalid v14.10 trace hash: {trace}")
...
def load_training_diagnostics(run_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    for arm in spec.ARMS:
        ...
        for environment in spec.ENVIRONMENTS:
            ...
            projection_by_level = {
                level: {"attempted": 0, "accepted": 0, "reductions": []}
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not isinstance(history, list) or not history:
                    raise ValueError("v14.10 training history is empty")
                ...
                if len(parameter_hash) != 64:
                    raise ValueError("v14.10 selected parameter hash is invalid")
                ...
                for level in ("upper", "lower"):
                    ...
                    attempted_rows = [
                        ...
                        if float(row.get(f"{prefix}_guard_attempted", 0.0)) > 0.5
                    ]
                    accepted_rows = [
                        row for row in attempted_rows
                        if float(row.get(f"{prefix}_guard_accepted", 0.0)) > 0.5
                    ]
                    reductions = [
                    ...
                    projection_by_level[level]["attempted"] += len(attempted_rows)
                    projection_by_level[level]["accepted"] += len(accepted_rows)
            ...
            by_environment[environment] = {
                ...
                "deployment_projection_by_level": {
                    level: {
                        ...
                        "guard_attempted_count": int(values["attempted"]),
                        "guard_accepted_count": int(values["accepted"]),
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "The v14.10 projection calibration is compared with a mean-reward "
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    if decision_path.exists() and decision_path.read_text(
        ...
        raise RuntimeError("existing v14.10 screen decision differs")
    ...
    lines = [
        "# MuJoCo v14.10 Deployment-Aligned Screen",
        "",
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_10_screen status={decision['status']} "
