#!/usr/bin/env python3
"""Analyze the frozen v14.28 restoration mechanism portfolio."""
...
from scripts.probe_mujoco_zeroth_order_actor_restoration import _eligible
...
def _validate_direction_gate(payload: dict[str, Any]) -> bool:
    ...
    if not isinstance(metrics, dict) or set(metrics) != {"upper", "lower"}:
        raise ValueError("v14.28 payload lacks both critic metric blocks")
    ...
    for level in ("upper", "lower"):
        ...
        if not isinstance(directions, dict) or level not in directions:
            raise ValueError("v14.28 payload lacks paired-direction metrics")
        ...
        for role, expected_count in (
            ...
            if not isinstance(values, dict):
                raise ValueError(f"v14.28 payload lacks {role} diagnostics")
            if values.get("estimator") != spec.PAIRED_DIRECTION_ESTIMATOR:
                raise ValueError(f"v14.28 {role} estimator drifted")
            if values.get("path_count") != expected_count:
                raise ValueError(f"v14.28 {role} direction count drifted")
            parameter_count = int(values["parameter_count"])
            if int(values["global_design_rank"]) != parameter_count:
                raise ValueError(f"v14.28 {role} global design is rank deficient")
            ...
            if not isinstance(ranks, dict) or len(ranks) != 4 or any(
                ...
                raise ValueError(f"v14.28 {role} mode design is rank deficient")
        if not isinstance(mode_cosines, dict) or len(mode_cosines) != 4:
            raise ValueError("v14.28 lacks four disturbance-mode cosines")
        ...
        if not math.isclose(
            ...
            raise ValueError("v14.28 minimum mode cosine is inconsistent")
        gate = gate and (
...
def _validate_payload(payload: dict[str, Any], environment: str, seed: int) -> None:
    ...
    if mismatches:
        raise ValueError(f"v14.28 payload contract mismatch: {mismatches}")
    fold_baselines = payload.get("design_fold_baselines")
    if not isinstance(fold_baselines, list) or len(fold_baselines) != 2:
        raise ValueError("v14.28 payload lacks two design-fold baselines")
    gate = _validate_direction_gate(payload)
    if bool(payload.get("critic_gate_pass")) != gate:
        raise ValueError("v14.28 critic/direction gate does not reproduce")
    candidates = payload.get("candidates")
    if not isinstance(candidates, list):
        raise ValueError("v14.28 payload lacks portfolio candidates")
    expected_count = spec.EXPECTED_ROUTER_CANDIDATE_COUNT + (
        spec.EXPECTED_ACTOR_CANDIDATE_COUNT if gate else 0
    )
    if int(payload.get("candidate_count", -1)) != expected_count:
        raise ValueError("v14.28 candidate count violates the direction gate")
    if len(candidates) != expected_count:
        raise ValueError("v14.28 candidate registry is incomplete")
    router_candidates = [
        candidate for candidate in candidates
        if candidate["source"] == "function_preserving_router_adapter"
    ]
    if sorted(candidate["router_strength"] for candidate in router_candidates) != sorted(
        spec.ROUTER_STRENGTH_VALUES
    ):
        raise ValueError("v14.28 router candidate registry drifted")
    for candidate in candidates:
        fold_snapshots = candidate.get("design_fold_snapshots")
        fold_flags = candidate.get("design_fold_eligible")
        if (
            not isinstance(fold_snapshots, list)
            or len(fold_snapshots) != 2
            or not isinstance(fold_flags, list)
            or len(fold_flags) != 2
        ):
            raise ValueError("v14.28 candidate lacks two-fold evidence")
        reproduced_folds = [
            _eligible(
                snapshot,
                baseline,
        ...
        if list(map(bool, fold_flags)) != reproduced_folds:
            raise ValueError("v14.28 fold eligibility does not reproduce")
        if bool(candidate["design_eligible"]) != bool(
            reproduced_pooled and all(reproduced_folds)
        ):
            raise ValueError("v14.28 pooled/fold candidate gate does not reproduce")
    if (
        payload.get("selected_design_candidate") is None
        and payload.get("validation_candidate") is not None
    ):
        raise ValueError("v14.28 validation candidate lacks design selection")
...
def analyze_run(run_name: str, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    for environment in spec.ENVIRONMENTS:
        for seed in spec.OPTIMIZER_SEEDS:
            ...
            if not path.is_file():
                raise FileNotFoundError(f"missing v14.28 probe result: {path}")
            ...
            source_counts = {
                source: sum(candidate["source"] == source for candidate in payload["candidates"])
                for source in sorted({candidate["source"] for candidate in payload["candidates"]})
            }
            rows.append({
                "environment": environment,
                "optimizer_seed": int(seed),
                "critic_direction_gate_pass": bool(payload["critic_gate_pass"]),
                "candidate_source_counts": json.dumps(source_counts, sort_keys=True),
                ...
                "selected_fold_eligible": (
                    None if selected is None else json.dumps(
                        selected["design_fold_eligible"]
                    )
                ),
    ...
    result = {
        "analysis_version": "mujoco_v14_28_mechanism_portfolio_preflight_v1",
        "evidence_role": spec.EVIDENCE_ROLE,
        "selection_contract": spec.SELECTION_CONTRACT,
        "cell_count": len(rows),
        "supported_cell_count": supported,
        "status": (
            "mechanism_portfolio_preflight_supported"
            if supported == spec.EXPECTED_CELL_COUNT
            else "mechanism_portfolio_preflight_not_supported"
        ),
        "cells": rows,
    }
    target = output_dir or (ROOT / "results" / run_name / "analysis")
    target.mkdir(parents=True, exist_ok=True)
    (target / "mechanism_portfolio_preflight.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    with (target / "mechanism_portfolio_preflight.csv").open(
