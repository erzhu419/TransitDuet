#!/usr/bin/env python3
"""Submit the frozen MuJoCo v14.5 paired-anchor screen via scheduleurm."""
...
SIGNATURE_VERSION = "mujoco-v14-5-paired-anchor-screen-v1"
ANCHOR_ARM = "direct_pretrain_anchor"
...
def runtime_identity() -> dict[str, str]:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v14.5 screen launcher/spec do not match HEAD")
    return {
...
def build_scheduler_spec(
    ...
    *,
    ...
    return {
        "project": "Freq-HRL-MuJoCo-v14.5-Paired-Anchor-Screen",
        "description": (
            f"Freq-HRL MuJoCo v14.5 {phase} {environment} {arm} "
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise RuntimeError("existing v14.5 preregistration differs")
...
def _read_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise SystemExit(f"v14.5 cell payload is invalid: {path}")
...
def merge_results(args: argparse.Namespace) -> None:
    ...
    if missing:
        raise SystemExit(
            f"cannot merge v14.5 screen: {len(missing)} cells missing; first={missing[0]}"
        )
        ...
        if phase == "anchor":
            ...
            checks = {
                **common_checks,
                ...
                == "frozen_matched_direct_policy_at_zero_router_context_analytic_gaussian_kl_v1",
    ...
    if issues:
        raise SystemExit("v14.5 screen merge failed: " + ",".join(issues[:20]))
    ...
    print(f"merged {len(cells)} frozen MuJoCo v14.5 cells")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v14.5 screen arm registry")
    if not args.phases or not set(args.phases).issubset(PHASES):
        raise SystemExit("invalid v14.5 screen phase registry")
    unknown_nodes = sorted(set(args.nodes) - set(LINUX_CPU_NODES))
    if unknown_nodes:
        raise SystemExit(f"invalid v14.5 screen nodes: {unknown_nodes}")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v14.5 frozen algorithm identity mismatch")
    ...
    if not cells:
        print("no v14.5 screen cells require submission")
        return
    print(
    ...
    execute_bulk([
        ...
        f"Freq-HRL MuJoCo v14.5 paired-anchor screen {args.run_name}"
    ))
