#!/usr/bin/env python3
"""Submit and sync the frozen MuJoCo v22 terminal-reserve confirmation."""
...
SIGNATURE_VERSION = "mujoco-v22-uniform-terminal-reserve-confirmation-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def selected_cells(args: argparse.Namespace) -> list[tuple[str, str, int]]:
    # Interleave paired arms and environments for each optimizer root so an
    # interrupted launch does not leave a coefficient- or domain-biased prefix.
    return [
...
def build_training_command(
    ...
    control_args = [
        ...
        *(str(seed) for seed in spec.EVALUATION_SEEDS),
        ...
        "--ppo-clip-ratio",
        str(spec.PPO_CLIP_RATIO),
        "--upper-projection-consistency-coef",
        str(arm_spec["upper_projection_consistency_coef"]),
        "--lower-projection-consistency-coef",
        str(arm_spec["lower_projection_consistency_coef"]),
        "--projection-consistency-update-mode",
        str(arm_spec["projection_consistency_update_mode"]),
        "--projection-consistency-weighting",
        str(arm_spec["projection_consistency_weighting"]),
        "--projection-consistency-advantage-temperature",
        str(arm_spec["projection_consistency_advantage_temperature"]),
        "--projection-consistency-advantage-weight-clip",
        str(arm_spec["projection_consistency_advantage_weight_clip"]),
        "--projection-consistency-step-scale",
        str(arm_spec["projection_consistency_step_scale"]),
        "--projection-consistency-max-backtracks",
        str(arm_spec["projection_consistency_max_backtracks"]),
        "--projection-consistency-reward-tolerance",
        str(arm_spec["projection_consistency_reward_tolerance"]),
        "--projection-consistency-training-schedule",
        str(arm_spec["projection_consistency_training_schedule"]),
        "--projection-consistency-warmup-fraction",
        str(arm_spec["projection_consistency_warmup_fraction"]),
        "--projection-consistency-ramp-fraction",
        str(arm_spec["projection_consistency_ramp_fraction"]),
        "--terminal-reserve-upper-window",
        str(spec.TERMINAL_RESERVE_UPPER_WINDOW),
        "--terminal-reserve-lower-window",
        str(spec.TERMINAL_RESERVE_LOWER_WINDOW),
        "--lower-lf-rms-budget",
    ...
    if bool(arm_spec["terminal_reserve_context"]):
        control_args.append("--terminal-reserve-context")
    if bool(arm_spec["terminal_reserve_projection"]):
        control_args.append("--terminal-reserve-projection")
    command = [
...
def build_scheduler_spec(
    ...
    return {
        "project": spec.CONFIRMATION_PROTOCOL_VERSION,
        "description": f"Freq-HRL v22 {environment} {arm} {optimizer_seed}",
        ...
        "cpu_training_justification": (
            "Independent v22 MuJoCo cells are CPU-bound and single-threaded."
        ),
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "confirmation_protocol_version": spec.CONFIRMATION_PROTOCOL_VERSION,
...
def _scheduler_tasks(run_name: str) -> dict[str, dict[str, Any]]:
    ...
    if json_start < 0:
        raise RuntimeError("scheduler results did not contain JSON")
    grouped: dict[str, list[dict[str, Any]]] = {}
    for task in json.loads(completed.stdout[json_start:]).get("results", []):
    ...
    for signature, attempts in grouped.items():
        ...
        if len(done) > 1:
            raise RuntimeError(f"multiple successful attempts for {signature}")
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v22 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v22 task is not done: {task.get('id')} status={task.get('status')}"
            )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v22 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v22 small-result cells")


def _verify_frozen_core() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("v22 frozen freq_hrl source differs from its revision")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v22 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v22 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v22 environment subset")
    if (
        ...
        raise SystemExit("invalid v22 optimizer seed subset")
    if not args.nodes or set(args.nodes) - set(LINUX_CPU_NODES):
        raise SystemExit("v22 accepts only node001-node006")
    if not args.python_executable.strip():
        args.python_executable = default_python_executable(args.nodes)
    if int(args.sync_workers) < 1:
        raise SystemExit("v22 sync workers must be positive")
    return args
...
def main() -> None:
    ...
    execute_bulk(
        ...
        intent_label=f"Freq-HRL {spec.CONFIRMATION_PROTOCOL_VERSION} {args.run_name}",
    )
