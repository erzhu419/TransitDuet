#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v19 development panel."""
...
SIGNATURE_VERSION = "mujoco-v19-terminal-reserve-training-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": f"Freq-HRL v19 {environment} {arm} {optimizer_seed}",
        ...
        "cpu_training_justification": (
            "Independent v19 MuJoCo cells are CPU-bound and single-threaded."
        ),
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v19 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v19 task is not done: {task.get('id')} status={task.get('status')}"
            )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v19 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v19 small-result cells")
...
def _verify_frozen_core() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("v19 frozen freq_hrl source differs from its revision")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v19 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v19 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v19 environment subset")
    if (
