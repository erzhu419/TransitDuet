#!/usr/bin/env python3
"""Submit one v17.10 horizon-reservoir FIR selector to node003."""
...
WORKER = Path("scripts/train_mujoco_v17_10_horizon_reservoir_fir.py")
SIGNATURE_VERSION = "mujoco-v17-10-horizon-reservoir-fir-selection-v1"
DATA_LOCAL_NODE = "node003"
CPU_JUSTIFICATION = (
    "The v17.10 selector performs deterministic NumPy ridge solves and causal "
    "energy-envelope projections over server-local paths; it has no GPU work."
)
...
def build_scheduler_spec(args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "description": "Freq-HRL v17.10 grouped horizon-reservoir selection",
...
def main() -> None:
    ...
    if args.cpu < 1 or args.ram_mb < 1024:
        raise SystemExit("v17.10 selection resources must be positive")
    ...
    if (
        ...
        raise SystemExit("v17.10 frozen core identity mismatch")
    execute_bulk(
