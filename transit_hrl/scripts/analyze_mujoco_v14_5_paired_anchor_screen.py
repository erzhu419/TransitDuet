#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v14.5 paired-anchor development screen."""
...
from scripts import mujoco_v14_5_paired_anchor_screen_spec as spec  # noqa: E402


ANALYSIS_VERSION = "mujoco_v14_5_paired_anchor_screen_analysis_v1"
METRICS = (
    "episode_return",
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.5 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    path_keys = [
    ...
    if len(set(path_keys)) != len(path_keys) or set(path_keys) != expected_keys:
        raise ValueError(f"v14.5 evaluation path registry mismatch: {path}")
    return rows
...
def _finite_mean(rows: list[dict[str, str]], metric: str) -> float:
    ...
    if values.size == 0 or not np.all(np.isfinite(values)):
        raise ValueError(f"non-finite v14.5 metric: {metric}")
...
def _validate_router_strength(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            f"v14.5 lower-action router strength mismatch: {cell_label}"
        )
...
def _bounds(
    ...
    *,
    ...
    if array.size < 2 or not np.all(np.isfinite(array)):
        raise ValueError("v14.5 bootstrap requires finite optimizer replicates")
...
def load_replicates(
    ...
    if (
        ...
        != "frozen_before_v14_5_development_outcome_access"
        ...
        raise ValueError("v14.5 preregistration identity mismatch")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("v14.5 analysis requires a complete merged manifest")
        ...
        for environment in spec.ENVIRONMENTS:
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not isinstance(history, list) or not history:
                    raise ValueError("v14.5 training history is empty")
                ...
                if not np.all(np.isfinite(upper_values)) or not np.all(
                    ...
                    raise ValueError("v14.5 anchor history contains non-finite values")
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    for arm in spec.CANDIDATE_ARMS:
        ...
        arm_status[arm] = {
            ...
            "development_selection_pass": bool(
                all_conditions and trained_pass and strict_pass
            ),
    ...
    return {
        ...
        "claim_boundary": (
            "The v14.5 screen may select one paired checkpoint continuation "
            "for a new v15 confirmation. Every candidate is compared with a "
            "compute-matched direct continuation from the same optimizer-state "
            "checkpoint. Responsibility drift must not worsen and raw physical "
            "lower-frequency drift must improve. This development screen cannot "
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    if decision_path.exists() and decision_path.read_text(
        ...
        raise RuntimeError("existing v14.5 screen decision differs")
    ...
    lines = [
        "# MuJoCo v14.5 Paired-Anchor Screen",
        "",
        ...
        "",
        "| arm | conditions | trained | strict | select | lower KL | lower param RMS | min slack |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_5_screen status={decision['status']} "
