#!/usr/bin/env python3
"""Submit Stage-8C compact plan-validity qualification."""
...
from scripts import pointmaze_compact_plan_validity_stage8c_spec as spec  # noqa: E402
...
CPU_JUSTIFICATION = (
    "Independent PointMaze PPO roots, paired deterministic branch replay, and "
    "grouped ridge fitting are single-threaded CPU workloads dynamically "
    "placed on the Linux pool."
)
...
def task_signature(
    ...
    return (
        f"Freq-HRL/{spec.EXPERIMENT_PROTOCOL}/{run_name}/{policy}/"
...
def training_command(
    ...
    *,
    ...
    command = [
        ...
        "--branch-window-seconds",
        str(options["branch_window_seconds"]),
        "--max-events-per-class",
        str(options["max_events_per_class"]),
        "--ridge-alpha-grid",
        *map(str, options["ridge_alpha_grid"]),
        "--selection-rate",
        str(options["selection_rate"]),
        "--regime-dwell-seconds",
        *map(str, options["regime_dwell_seconds"]),
        "--target-speed-modes",
        *map(str, options["target_speed_modes"]),
        "--force-pulse-amplitude",
        str(options["force_pulse_amplitude"]),
        "--force-pulse-duration-seconds",
        *map(str, options["force_pulse_duration_seconds"]),
        "--force-pulse-gap-seconds",
        *map(str, options["force_pulse_gap_seconds"]),
        "--distractor-amplitude",
        str(options["distractor_amplitude"]),
        ...
        *map(str, options["selection"]),
        "--branch-fit-seeds",
        *map(str, options["branch_fit"]),
        "--branch-eval-seeds",
        *map(str, options["branch_eval"]),
        "--output",
        str(output),
    ]
    environment = (
...
def task_specification(
    ...
    *,
    ...
    policy, optimizer_seed = cell
    relative = cell_relative_dir(run_name, policy, optimizer_seed)
    phase = "preflight" if preflight else "development"
    return {
        "project": spec.EXPERIMENT_PROTOCOL,
        "description": (
            f"Freq-HRL PointMaze stage8c {phase} {policy} root{optimizer_seed}"
        ),
        "cmd": training_command(run_name, cell, preflight=preflight),
        "cwd": str(ROOT),
        "signature": task_signature(run_name, policy, optimizer_seed),
...
def sync_results(
    ...
    *,
    ...
    for policy, optimizer_seed in spec.cells(preflight=preflight):
        signature = task_signature(run_name, policy, optimizer_seed)
        task = tasks.get(signature)
        if task is None:
            raise SystemExit(f"Stage-8C sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"Stage-8C task is not done: {task.get('id')} "
                f"status={task.get('status')}"
            )
        path = ROOT / cell_relative_dir(run_name, policy, optimizer_seed)
    ...
    if pending:
        ...
        raise SystemExit(
            f"Stage-8C result sync incomplete: {len(pending)} cells; "
    ...
    for signature, path, _ in expected:
        ...
        if (
            ...
            raise SystemExit(f"invalid synced Stage-8C result: {signature}")
    ...
    print(f"synced {len(expected)} Stage-8C result JSON files")
...
def main() -> int:
    ...
    registration = {
        ...
        "runtime_protocol": spec.PROTOCOL,
        "algorithm_revision": spec.ALGORITHM_REVISION,
        "evidence_stage": "preflight" if args.preflight else spec.EVIDENCE_STAGE,
        ...
        "claim_gate": dict(spec.CLAIM_GATE),
        "development_provenance": {
            "source": "completed Stage-8B formal results",
            "role": "structure_selection_only_not_confirmation",
        },
        "scheduler": {
    ...
    execute_bulk(
        [
            task_specification(
                args.run_name, cell, preflight=args.preflight
            )
            for cell in cells
        ],
