#!/usr/bin/env python3
"""Audit the scoped single-seed MuJoCo v14.12 preflight."""
...
ANALYSIS_VERSION = "mujoco_v14_12_groupwise_robust_preflight_analysis_v1"
FREQUENCY_METRICS = (
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _load_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise ValueError(f"invalid v14.12 preflight cell: {path}")
...
def _mode_payload(rows: list[dict[str, str]], mode: str) -> dict[str, Any]:
    ...
    if len(selected) != len(spec.DEVELOPMENT_EVALUATION_SEEDS):
        raise ValueError(f"v14.12 preflight mode is incomplete: {mode}")
    payload = {
        metric: (
...
def analyze_preflight(
    ...
    if environment not in spec.ENVIRONMENTS:
        raise ValueError(f"invalid v14.12 preflight environment: {environment}")
    if int(optimizer_seed) not in spec.OPTIMIZER_SEEDS:
        raise ValueError("invalid v14.12 preflight optimizer seed")
    ...
    identity_pass = bool(
        ...
        == "frozen_before_v14_12_groupwise_robust_outcome_access"
    ...
    if not identity_pass:
        raise ValueError("v14.12 preflight scope or frozen identity mismatch")
    ...
    if not (
        ...
        raise ValueError("v14.12 preflight anchor identity mismatch")
    ...
    for arm in spec.ARMS:
        ...
        if not (
            ...
            raise ValueError(f"v14.12 preflight cell identity mismatch: {arm}")
        ...
        if not (
            ...
            raise ValueError(f"v14.12 preflight anchor provenance mismatch: {arm}")
        if (
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.12 preflight paired baseline mismatch: {arm}"
                )
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.12 checkpoint diagnostics are incomplete: {arm}"
                )
        cells[arm] = cell
    ...
    for arm, status in arm_status.items():
        groupwise = arm in spec.GROUPWISE_ARMS
        passed = bool(status["base_preflight_pass"] and groupwise)
        status.update({
            "groupwise_candidate": groupwise,
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_12_preflight status={decision['status']} "
