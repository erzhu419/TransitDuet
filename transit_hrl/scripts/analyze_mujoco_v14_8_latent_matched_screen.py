#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v14.8 latent matched-control screen."""
...
from scripts import mujoco_v14_8_latent_matched_screen_spec as spec  # noqa: E402


ANALYSIS_VERSION = "mujoco_v14_8_latent_matched_screen_analysis_v1"
METRICS = (
    "episode_return",
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.8 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    ...
    if len(set(keys)) != len(keys) or set(keys) != expected_keys:
        raise ValueError(f"v14.8 evaluation path registry mismatch: {path}")
    return rows
...
def _finite_mean(rows: list[dict[str, str]], metric: str) -> float:
    ...
    if values.size == 0 or not np.all(np.isfinite(values)):
        raise ValueError(f"non-finite v14.8 metric: {metric}")
...
def _bounds(
    ...
    *,
    ...
    if array.size < 2 or not np.all(np.isfinite(array)):
        raise ValueError("v14.8 bootstrap requires finite optimizer replicates")
...
def _actor_vectors(path: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not isinstance(model, dict):
        raise ValueError(f"invalid v14.8 checkpoint payload: {path}")
    vectors: list[np.ndarray] = []
    for actor_name in ("upper_actor", "lower_actor"):
        actor = model.get(actor_name)
        if not isinstance(actor, dict) or not actor:
            raise ValueError(f"missing v14.8 checkpoint actor {actor_name}: {path}")
        values = []
        for name in sorted(actor):
            tensor = actor[name]
            if not torch.is_tensor(tensor):
                raise ValueError(
                    f"non-tensor v14.8 actor parameter {actor_name}.{name}"
                )
            values.append(
...
def _actor_rms_difference(candidate: Path, comparator: Path) -> dict[str, float]:
    ...
    if (
        ...
        raise ValueError("v14.8 paired actor checkpoint shapes differ")
...
def load_replicates(
    ...
    if (
        ...
        != "frozen_before_v14_8_development_outcome_access"
        ...
        raise ValueError("v14.8 preregistration identity mismatch")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("v14.8 analysis requires a complete merged manifest")
        ...
        for arm, arm_spec in spec.ARMS.items():
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not np.allclose(
                    ...
                    raise ValueError(
                        f"v14.8 router strength mismatch: {environment}/{arm}/{seed}"
                    )
                for mode in spec.EVALUATION_DISTURBANCE_MODES:
                    mode_rows = [
                    ...
                    for trace in TRACE_KEYS:
                        ...
                        if any(len(value) != 64 for value in values):
                            raise ValueError(f"invalid v14.8 trace hash: {trace}")
        ...
        for environment in spec.ENVIRONMENTS:
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not isinstance(history, list) or not history:
                    raise ValueError("v14.8 training history is empty")
                selected = int(summary.get("selected_checkpoint_iteration", -2))
                trained.append(float(
                    selected >= spec.CONTINUATION_CHECKPOINT_MINIMUM_ITERATION
                ))
                parameter_hash = str(summary.get("frozen_parameter_sha256", ""))
                if len(parameter_hash) != 64:
                    raise ValueError("v14.8 selected parameter hash is invalid")
                hashes[str(seed)] = parameter_hash
                upper_values = [
                    float(row["upper_constraint_lambda"])
                    for row in history if "upper_constraint_lambda" in row
                ]
                lower_values = [
                    float(row["lower_constraint_lambda"])
                    for row in history if "lower_constraint_lambda" in row
                ]
                upper_lambda.append(upper_values[-1] if upper_values else 0.0)
                lower_lambda.append(lower_values[-1] if lower_values else 0.0)
            by_environment[environment] = {
                "trained_checkpoint_fraction": float(np.mean(trained)),
                "upper_constraint_lambda_final_mean": float(
                    np.mean(upper_lambda)
                ),
                "lower_constraint_lambda_final_mean": float(
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "The v14.8 projection calibration is compared with a mean-reward "
            ...
            "selected. Every learned arm is compared with the same-strength, "
            "same-selector zero-dual control and must change actor tensors and "
            "actions, retain reward noninferiority in every condition, strictly "
            "improve reward in at least one condition, reduce effective and "
            "latent frequency leakage, preserve lower activity, and satisfy "
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    if decision_path.exists() and decision_path.read_text(
        ...
        raise RuntimeError("existing v14.8 screen decision differs")
    ...
    lines = [
        "# MuJoCo v14.8 Latent Matched-Control Screen",
        "",
        ...
        "- Learned comparator: same projection and latent-aware selector, zero dual.",
        "",
        "| arm | role | comparator | conditions | trained | actor gate | behavior | strict reward | select | paired actor RMS min | min slack |",
        "|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_8_screen status={decision['status']} "
