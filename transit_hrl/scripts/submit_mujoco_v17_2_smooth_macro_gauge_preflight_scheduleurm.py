#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v17.2 paired preflight."""
...
WORKER = Path("scripts/run_mujoco_v17_2_paired_gauge_cell.py")
SIGNATURE_VERSION = "mujoco-v17-2-smooth-macro-gauge-preflight-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def selected_cells(args: argparse.Namespace) -> list[tuple[str, str, int]]:
    return [
        (environment, alpha_arm, int(seed))
        for environment in args.environments
        for alpha_arm in args.alpha_arms
...
def task_signature(
    ...
    return (
        f"Freq-HRL/{SIGNATURE_VERSION}/{run_name}/{environment}/{alpha_arm}/"
...
def build_training_command(
    ...
    full_output = full_cell_relative_dir(
        args.run_name, environment, alpha_arm, optimizer_seed
    )
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": (
            f"Freq-HRL v17.2 {environment} {alpha_arm} {optimizer_seed}"
        ),
        "cmd": build_training_command(
            args, environment, alpha_arm, optimizer_seed
        ),
        "cwd": str(ROOT),
        ...
        "resource_family": (
            f"Freq-HRL/{SIGNATURE_VERSION}/{alpha_arm}/{environment}/cell"
        ),
        "vram": 0,
        "ram_mb": 1024,
        "cpu": 1,
...
def _write_preregistration(args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "alpha_arms": {
            arm: spec.ALPHA_ARMS[arm] for arm in args.alpha_arms
        },
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, alpha_arm, seed in selected_cells(args):
        signature = task_signature(
            args.run_name, environment, alpha_arm, seed
        )
        task = tasks.get(signature)
        if task is None:
            raise SystemExit(f"v17.2 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                "v17.2 task is not done: "
    ...
    if pending:
        ...
        raise SystemExit(
            f"v17.2 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v17.2 small-result cells")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--run-name", required=True)
    parser.add_argument("--alpha-arms", default=",".join(spec.ALPHA_ARMS))
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    args.alpha_arms = parse_csv(args.alpha_arms)
    ...
    try:
        ...
        raise SystemExit("invalid v17.2 optimizer seed subset") from exc
    if not args.alpha_arms or not set(args.alpha_arms).issubset(spec.ALPHA_ARMS):
        raise SystemExit("invalid v17.2 alpha subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v17.2 environment subset")
    if (
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v17.2 frozen algorithm identity mismatch")
    if args.sync_only:
        sync_results(args)
        return
