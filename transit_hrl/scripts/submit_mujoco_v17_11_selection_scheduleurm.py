#!/usr/bin/env python3
"""Submit one v17.11 fractional-reservoir selector to node003."""
...
WORKER = Path("scripts/train_mujoco_v17_11_fractional_reservoir_fir.py")
SIGNATURE_VERSION = "mujoco-v17-11-fractional-reservoir-fir-selection-v1"
DATA_LOCAL_NODE = "node003"
CPU_JUSTIFICATION = (
    "The v17.11 selector performs deterministic NumPy ridge solves and causal "
    "fractional energy-envelope projections over server-local paths; it has "
    "no GPU work."
)
...
def build_scheduler_spec(args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "description": "Freq-HRL v17.11 grouped fractional-reservoir screen",
...
def main() -> None:
    ...
    if args.cpu < 1 or args.ram_mb < 1024:
        raise SystemExit("v17.11 selection resources must be positive")
    ...
    if (
        ...
        raise SystemExit("v17.11 frozen core identity mismatch")
    execute_bulk(
