#!/usr/bin/env python3
"""Analyze the frozen MuJoCo v14.4 router-homotopy development screen."""
...
ANALYSIS_VERSION = "mujoco_v14_4_router_homotopy_screen_analysis_v1"
METRICS = (
    "episode_return",
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.4 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    path_keys = [
    ...
    if len(set(path_keys)) != len(path_keys) or set(path_keys) != expected_keys:
        raise ValueError(f"v14.4 evaluation path registry mismatch: {path}")
    return rows
...
def _finite_mean(rows: list[dict[str, str]], metric: str) -> float:
    ...
    if values.size == 0 or not np.all(np.isfinite(values)):
        raise ValueError(f"non-finite v14.4 metric: {metric}")
...
def _validate_router_strength(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            f"v14.4 lower-action router strength mismatch: {cell_label}"
        )
...
def _bounds(
    ...
    *,
    ...
    if array.size < 2 or not np.all(np.isfinite(array)):
        raise ValueError("v14.4 bootstrap requires finite optimizer replicates")
...
def load_replicates(
    ...
    if (
        ...
        != "frozen_before_v14_4_development_outcome_access"
        ...
        raise ValueError("v14.4 preregistration identity mismatch")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("v14.4 analysis requires a complete merged manifest")
        ...
        for environment in spec.ENVIRONMENTS:
            ...
            for seed in spec.OPTIMIZER_SEEDS:
                ...
                if not isinstance(history, list) or not history:
                    raise ValueError("v14.4 training history is empty")
                ...
                if (
                    ...
                    raise ValueError("v14.4 dual history contains non-finite values")
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "The v14.4 screen may select one fully trained router-homotopy arm "
            "for a new v15 confirmation. Responsibility transfer is held fixed "
            "and tested for no worsening; raw physical lower-frequency drift "
            "must improve. Absolute at-floor conditions establish "
            "noninferiority only. This development screen cannot support a "
            "confirmatory claim."
        ),
    }
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    if decision_path.exists() and decision_path.read_text(
        ...
        raise RuntimeError("existing v14.4 screen decision differs")
    ...
    lines = [
        "# MuJoCo v14.4 Router-Homotopy Screen",
        "",
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_4_screen status={decision['status']} "
