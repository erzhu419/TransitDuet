#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v21 mechanism preflight."""
...
SIGNATURE_VERSION = "mujoco-v21-reward-selective-feasible-action-preflight-v1"
SMALL_RESULT_FILES = (
    "cell_summary.json",
    "evaluation_rows.csv",
...
def build_scheduler_spec(
    ...
    return {
        ...
        "description": f"Freq-HRL v21 {environment} {arm} {optimizer_seed}",
        ...
        "cpu_training_justification": (
            "Independent v21 MuJoCo cells are CPU-bound and single-threaded."
        ),
...
def sync_results(args: argparse.Namespace) -> None:
    ...
    for environment, arm, seed in selected_cells(args):
        ...
        if task is None:
            raise SystemExit(f"v21 sync task missing: {signature}")
        if task.get("status") != "done" or not task.get("node"):
            raise SystemExit(
                f"v21 task is not done: {task.get('id')} status={task.get('status')}"
            )
    ...
    if pending:
        ...
        raise SystemExit(
            f"v21 result sync incomplete: {len(pending)} cells; "
    ...
    print(f"synced {len(expected)} v21 small-result cells")
...
def _verify_frozen_core() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("v21 frozen freq_hrl source differs from its revision")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        raise SystemExit("invalid v21 optimizer seed subset") from exc
    if not args.arms or not set(args.arms).issubset(spec.ARMS):
        raise SystemExit("invalid v21 arm subset")
    if not args.environments or not set(args.environments).issubset(
        spec.ENVIRONMENTS
    ):
        raise SystemExit("invalid v21 environment subset")
    if (
