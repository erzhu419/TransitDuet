#!/usr/bin/env python3
"""Submit and synchronize the frozen MuJoCo v15 development preflight."""
...
from scripts.analyze_mujoco_v15_raw_policy_distillation import analyze_run
from scripts import submit_mujoco_v14_18_router_probe_scheduleurm as base


ROOT = Path(__file__).resolve().parents[1]
LAUNCHER_PATH = Path(__file__).resolve()
SIGNATURE_VERSION = "mujoco-v15-raw-policy-distillation-preflight-v1"
...
def _build_probe_command(args, environment: str, seed: int) -> str:
    ...
    command = [
        ...
        "scripts/probe_mujoco_raw_policy_distillation.py",
...
def _build_scheduler_spec(args, environment: str, seed: int):
...
def _write_preregistration(args) -> None:
    ...
    payload = {
        "status": "frozen_before_v15_development_outcome_access",
...
def _normalize_args(args):
    ...
    if str(normalized.anchor_run_name) != spec.ANCHOR_RUN_NAME:
        raise SystemExit(f"v15 anchor run is frozen as {spec.ANCHOR_RUN_NAME}")
    qualification = analyze_anchor_run(normalized.anchor_run_name)
    if qualification.get("status") != "fresh_anchor_bank_qualified":
        raise SystemExit("v15 requires the qualified v14.29 fresh anchor bank")
    return normalized
