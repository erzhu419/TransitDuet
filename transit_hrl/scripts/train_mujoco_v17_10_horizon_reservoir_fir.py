#!/usr/bin/env python3
"""Run grouped v17.10 horizon-reservoir FIR selection on reused paths."""
...
from scripts import (  # noqa: E402
    mujoco_v17_10_horizon_reservoir_fir_spec as spec,
)
...
def candidate_configs() -> list[dict[str, Any]]:
    return [
        {
            "candidate_id": (
                f"reservoir{int(reserve)}_fir_w{int(window)}_"
            ...
            "router_mode": "horizon_reservoir_hpf_innovation_projection",
        }
...
def grouped_out_of_fold_rows(
        ...
        for held_seed in spec.REUSED_SELECTION_SEEDS:
            ...
            if (
                ...
                * len(spec.DISTURBANCE_MODES)
                ...
                raise RuntimeError("v17.10 grouped fold construction mismatch")
    ...
    for candidate, rows in rows_by_candidate.items():
        if len(rows) != spec.REUSED_EXPECTED_PATH_COUNT:
            raise RuntimeError(
                f"v17.10 OOF path count mismatch for {candidate}"
            )
...
def run_selection(dataset_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    horizon_gate = bool(all(
        row["minimum_horizon_certified"] for row in selected_rows
    ))
    gate["minimum_horizon_certified_on_all_paths"] = horizon_gate
    advances = bool(all(gate.values()))
    summary = {
        "status": (
            "horizon_reservoir_fir_advances_to_fresh_path_validation"
            if advances
            else "horizon_reservoir_fir_stops_before_fresh_path_access"
        ),
    ...
    model = {
        "status": "v17_10_selected_horizon_reservoir_fir_model_fitted",
        "development_protocol_version": spec.DEVELOPMENT_PROTOCOL_VERSION,
        "frozen_core_revision": spec.FROZEN_CORE_REVISION,
        "frozen_source_manifest_sha256": spec.FROZEN_SOURCE_MANIFEST_SHA256,
        "selected_candidate_id": selected["candidate_id"],
...
def main() -> None:
    ...
    print(
        f"DONE v17.10 selection status={summary['status']} "
