#!/usr/bin/env python3
"""Integrity-first analysis for the preregistered MuJoCo v12 comparison."""
...
ANALYSIS_VERSION = "mujoco_v12_confirmatory_analysis_v1"
...
def _bootstrap_environment(
    *,
    ...
    rng = np.random.default_rng(derive_seed(
        "mujoco_v12_confirmatory_primary_bootstrap",
        environment,
    ))
    ...
    return {
        ...
        "relative_drift_reduction": drift_reduction,
        "relative_drift_reduction_ci95_lower": float(drift_ci95[0]),
        "relative_drift_reduction_ci95_upper": float(drift_ci95[1]),
        "drift_reduction_familywise_lower_bound": drift_reduction_lower,
        "minimum_drift_reduction_pass": bool(
            drift_reduction_lower >= spec.MINIMUM_DRIFT_REDUCTION_FRACTION
        ),
    }
...
def analyze(
    ...
    *,
    ...
    if any(audit["status"] != "valid" for audit in audits.values()):
        raise ValueError("MuJoCo v12 confirmatory integrity audit failed")
    runtime_identities = {
    ...
    for environment in spec.ENVIRONMENTS:
        ...
        row["environment_primary_gate_pass"] = bool(
            ...
            and row["minimum_drift_reduction_pass"]
    ...
    payload = {
        ...
        "selection_rule": (
            ...
            "noninferiority_and_drift_reduction_plus_reconstruction_integrity"
        ),
    }
    ...
    for row in environment_rows:
        table.append(
            ...
            f"{100.0 * row['relative_drift_reduction']:.2f}% "
            f"[{100.0 * row['relative_drift_reduction_ci95_lower']:.2f}%, "
            f"{100.0 * row['relative_drift_reduction_ci95_upper']:.2f}%] | "
            f"{row['minimum_drift_reduction_pass']} |"
        )
    (output / "report.md").write_text("\n".join([
        ...
        f"- family-wise alpha: `{spec.FAMILY_WISE_ALPHA}` across `{spec.PRIMARY_GATE_COUNT}` statistical gates",
        "",
        *table,
        "",
...
def main() -> None:
    ...
    print(f"mujoco_v12_confirmatory status={payload['status']}")
