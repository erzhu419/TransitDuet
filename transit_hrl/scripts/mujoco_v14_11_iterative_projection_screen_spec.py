"""Frozen design for the MuJoCo v14.11 iterative-projection screen."""
...
FROZEN_ALGORITHM_REVISION = "ed87b9a0d3e2b78a9c7c10fd76291c64af246564"
...
ARMS = {
    ...
    "relative_s050_asym_u003_l008_s13_r05_k1": _continuation_arm(
        role="learned",
        router_strength=0.50,
    ...
    "relative_s050_asym_u003_l008_s13_r05_k4": _continuation_arm(
        role="learned",
        router_strength=0.50,
        upper_deployment_dual_lr=0.03,
        lower_deployment_dual_lr=0.08,
    ...
    "relative_s050_asym_u003_l008_s13_r05_k8": _continuation_arm(
        role="learned",
        router_strength=0.50,
        upper_deployment_dual_lr=0.03,
        lower_deployment_dual_lr=0.08,
    ...
    "relative_s050_asym_u003_l008_s13_r05_k16": _continuation_arm(
        role="learned",
        router_strength=0.50,
        upper_deployment_dual_lr=0.03,
        lower_deployment_dual_lr=0.08,
    ...
    "relative_s050_asym_u003_l008_s310_r05_k8": _continuation_arm(
        role="learned",
        router_strength=0.50,
        upper_deployment_dual_lr=0.03,
        lower_deployment_dual_lr=0.08,
    ...
    "relative_s050_asym_u003_l008_s310_r10_k8": _continuation_arm(
        role="learned",
        router_strength=0.50,
        upper_deployment_dual_lr=0.03,
        lower_deployment_dual_lr=0.08,
...
LEGACY_ONE_STEP_ARM = "relative_s050_asym_u003_l008_s13_r05_k1"
ITERATIVE_ARMS = tuple(
    arm for arm in LEARNED_ARMS
    if int(ARMS[arm]["upper_deployment_frequency_max_projection_steps"]) > 1
)

# Fresh development-only namespace. A confirmatory v15 must use new values.
OPTIMIZER_SEEDS = (
    2619431165, 84392227, 1187350958, 1002167899,
    3416184012, 3320221953, 850848356, 3708610200,
    173006395, 3149021542, 1293317206, 1998003191,
    3533803580, 1002654473, 93078986, 1480011597,
)
PRETRAIN_SEEDS = (1042461199, 1703748658, 4270117286, 4124505478)
PRETRAIN_SELECTION_SEEDS = (2369087478, 4198394466)
CONTINUATION_TRAIN_SEEDS = (3190109880, 1656313776, 25620928, 1138183376)
CONTINUATION_SELECTION_SEEDS = (1721174499, 1188385153)
DEVELOPMENT_EVALUATION_SEEDS = (
    1453284150, 3695705041, 3821186372, 966140989,
    3870581487, 1338308038, 2150158927, 1259110201,
)
...
MINIMUM_ITERATIVE_ACCEPTED_STEPS = 2
MINIMUM_ITERATIVE_MULTISTEP_UPDATES = 1
MINIMUM_ITERATIVE_REDUCTION_GAIN_OVER_ONE_STEP = 0.01
...
DEVELOPMENT_DISCLOSURE = (
    "This v14.11 screen was designed after v14.10 validated exact deployment "
    "frequency accounting and causal router calibration, but every learned arm "
    "fell back to the initial checkpoint. The accepted one-step corrections "
    "reduced within-update frequency power by less than one percent and PPO "
    "drift restored target violations at checkpoint selection. This screen "
    "holds the paired anchor, dual rates, router, target, and seed fixed while "
    "varying the number and scale of reward-budgeted projection steps. The "
    ...
    "checkpoint. Every projection step shares one cumulative reward-loss "
    "budget; per-step reward tolerance cannot compound. "
...
def validate_frozen_design() -> None:
    ...
    if len(flattened) != len(set(flattened)):
        raise ValueError("v14.11 seed roles overlap")
    if len(OPTIMIZER_SEEDS) != 16:
        raise ValueError("v14.11 requires 16 optimizer replicates")
    if ARMS[CALIBRATION_ARM]["checkpoint_score_mode"] != "mean_reward":
        raise ValueError("v14.11 calibration must use mean-reward selection")
    if len(ARMS) != 9:
        raise ValueError("v14.11 requires exactly 9 continuation arms")
    ...
    if (
        ...
        raise ValueError("v14.11 matched comparator contract drifted")
    for arm in LEARNED_ARMS:
        ...
        if (
            ...
            raise ValueError(f"v14.11 learned arm is not matched: {arm}")
        for level in ("upper", "lower"):
            ...
            if (dual > 0.0) != (reduction > 0.0):
                raise ValueError(
                    f"v14.11 {level} deployment target is not active-matched: {arm}"
                )
        ...
        if projection_steps != int(
            ...
            raise ValueError(f"v14.11 projection-step levels differ: {arm}")
        for level in ("upper", "lower"):
            tolerance = float(
                arm_spec[f"{level}_deployment_frequency_reward_tolerance"]
            )
            if tolerance != 1e-4:
                raise ValueError(f"v14.11 reward budget drifted: {arm}")
    if int(ARMS[LEGACY_ONE_STEP_ARM][
        "upper_deployment_frequency_max_projection_steps"
    ]) != 1:
        raise ValueError("v14.11 legacy comparator must remain one-step")
    if not ITERATIVE_ARMS or any(
        int(ARMS[arm]["upper_deployment_frequency_max_projection_steps"]) <= 1
        for arm in ITERATIVE_ARMS
    ):
        raise ValueError("v14.11 iterative arm contract drifted")
