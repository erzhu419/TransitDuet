#!/usr/bin/env python3
"""Analyze the frozen v14.22 action-cost critic preflight."""
...
def _validate_payload(payload: dict[str, Any], environment: str, seed: int) -> None:
    ...
    if mismatches:
        raise ValueError(f"v14.22 payload contract mismatch: {mismatches}")
    ...
    if not isinstance(metrics, dict) or set(metrics) != {"upper", "lower"}:
        raise ValueError("v14.22 payload lacks both critic metric blocks")
    critic_gate_expected = payload.get("gradient_error") is None
    for level in ("upper", "lower"):
        level_metrics = metrics[level]
        critic_gate_expected = critic_gate_expected and (
            float(level_metrics["ensemble_holdout_r2"])
            > spec.CRITIC_MINIMUM_HOLDOUT_R2
            and float(level_metrics[
        ...
        if payload.get("gradient_error") is None:
            if not isinstance(gradients, dict) or level not in gradients:
                raise ValueError("v14.22 payload lacks actor-gradient metrics")
            critic_gate_expected = critic_gate_expected and (
                float(gradients[level]["median_gradient_cosine"])
                > spec.MINIMUM_GRADIENT_MEDIAN_COSINE
            )
    if bool(payload.get("critic_gate_pass")) != bool(critic_gate_expected):
        raise ValueError("v14.22 critic gate does not reproduce from metrics")
    expected_candidates = (
        spec.EXPECTED_CANDIDATE_COUNT if critic_gate_expected else 0
    )
    if int(payload.get("candidate_count", -1)) != expected_candidates:
        raise ValueError("v14.22 candidate count violates critic gate")
    selected = payload.get("selected_design_candidate")
    if selected is None and payload.get("validation_candidate") is not None:
        raise ValueError("v14.22 validation candidate lacks design selection")
    if bool(payload.get("validation_supported")) and selected is None:
        raise ValueError("v14.22 cannot support validation without selection")
...
def analyze_run(run_name: str, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    for environment in spec.ENVIRONMENTS:
        for seed in spec.OPTIMIZER_SEEDS:
            ...
            if not path.is_file():
                raise FileNotFoundError(f"missing v14.22 probe result: {path}")
            ...
            baseline_merit = _metric(
                validation_baseline, "frequency_violation_merit"
            )
            candidate_merit = _metric(
    ...
    result = {
        "analysis_version": "mujoco_v14_22_action_cost_critic_preflight_v1",
        "evidence_role": spec.EVIDENCE_ROLE,
        "selection_contract": spec.SELECTION_CONTRACT,
        "cell_count": len(rows),
        "supported_cell_count": supported,
        "status": (
            "action_cost_critic_preflight_supported"
            if supported == spec.EXPECTED_CELL_COUNT
            else "action_cost_critic_preflight_not_supported"
        ),
        "cells": rows,
    }
    target = output_dir or (ROOT / "results" / run_name / "analysis")
    target.mkdir(parents=True, exist_ok=True)
    (target / "action_cost_critic_preflight.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    with (target / "action_cost_critic_preflight.csv").open(
