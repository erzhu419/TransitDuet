#!/usr/bin/env python3
"""Audit the scoped single-seed MuJoCo v14.15 preflight."""
...
from scripts.submit_mujoco_v14_15_closed_loop_restoration_filter_screen_scheduleurm import (  # noqa: E402
    _closed_loop_guard_contract_valid,
)


ANALYSIS_VERSION = "mujoco_v14_15_closed_loop_restoration_filter_preflight_analysis_v1"
FREQUENCY_METRICS = (
    "LowerLFDriftAbs",
    "RawLowerLFDriftAbs",
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != expected:
        raise ValueError(
            f"v14.15 cell has {len(rows)} rows; expected {expected}: {path}"
        )
    ...
    if len(set(keys)) != len(keys) or set(keys) != expected_keys:
        raise ValueError(f"v14.15 evaluation path registry mismatch: {path}")
    return rows
...
def _load_cell(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not isinstance(summary, dict) or not isinstance(history, list) or not history:
        raise ValueError(f"invalid v14.15 preflight cell: {path}")
...
def _mode_payload(rows: list[dict[str, str]], mode: str) -> dict[str, Any]:
    ...
    if len(selected) != len(spec.DEVELOPMENT_EVALUATION_SEEDS):
        raise ValueError(f"v14.15 preflight mode is incomplete: {mode}")
    payload = {
        metric: (
...
def _closed_loop_guard_diagnostics(
    ...
    contract_valid = _closed_loop_guard_contract_valid(
        ...
        history,
        expected=expected,
        restoration_expected=restoration_expected,
    ...
    initial_merit = float(summary.get(
        f"{prefix}initial_frequency_violation_merit", 0.0
    ))
    training_final_merit = float(summary.get(
    ...
    initial_worst = float(summary.get(
        f"{prefix}initial_worst_frequency_violation", 0.0
    ))
    training_final_worst = float(summary.get(
    ...
    maintenance_reached = bool(any(
        str(row.get(f"{prefix}restoration_phase_after", "")) == "maintenance"
        for row in history
    )) if restoration_expected else False
    passed = bool(
        contract_valid
        and (
            not expected
    ...
    return {
        ...
        "initial_frequency_violation_merit": initial_merit,
        "training_final_frequency_violation_merit": training_final_merit,
        "selected_frequency_violation_merit": selected_merit,
        "initial_worst_frequency_violation": initial_worst,
        "training_final_worst_frequency_violation": training_final_worst,
        "selected_worst_frequency_violation": selected_worst,
        "restoration_merit_reduction_fraction": float(
            1.0 - training_final_merit / max(initial_merit, 1e-12)
        ) if restoration_expected else 0.0,
        "restoration_maintenance_reached": maintenance_reached,
        "restoration_funnel_limit": float(summary.get(
            "deployment_frequency_closed_loop_restoration_funnel_limit", 0.0
        )),
        "pass": passed,
    }
...
def analyze_preflight(
    ...
    *,
    ...
    if environment not in spec.ENVIRONMENTS:
        raise ValueError(f"invalid v14.15 preflight environment: {environment}")
    if int(optimizer_seed) not in spec.OPTIMIZER_SEEDS:
        raise ValueError("invalid v14.15 preflight optimizer seed")
    ...
    if not identity_pass:
        raise ValueError("v14.15 preflight scope or frozen identity mismatch")
    ...
    for arm in spec.ARMS:
        ...
        if not (
            ...
            raise ValueError(f"v14.15 preflight anchor provenance mismatch: {arm}")
        if (
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.15 preflight paired baseline mismatch: {arm}"
                )
            ...
            if not (
                ...
                raise ValueError(
                    f"v14.15 checkpoint diagnostics are incomplete: {arm}"
                )
        cells[arm] = cell
    ...
    for arm, status in arm_status.items():
        ...
        status.update({
            ...
            "restoration_candidate": arm in spec.RESTORATION_ARMS,
...
def write_analysis(output_dir: Path, decision: dict[str, Any]) -> None:
    ...
    for arm, status in decision["arm_status"].items():
        lines.append(
            f"| {arm} | {status['restoration_candidate']} | "
            f"{status['closed_loop_guard']['effective_update_count']} | "
            f"{status['closed_loop_guard']['selected_reward_violation_count']}/"
            f"{status['closed_loop_guard']['selected_frequency_violation_count']} | "
            f"{status['closed_loop_guard']['initial_frequency_violation_merit']:.6g}/"
            f"{status['closed_loop_guard']['training_final_frequency_violation_merit']:.6g}/"
            f"{status['closed_loop_guard']['selected_frequency_violation_merit']:.6g} | "
            f"{status['trained_checkpoint_pass']} | "
...
def main() -> None:
    ...
    print(
        f"mujoco_v14_15_preflight status={decision['status']} "
