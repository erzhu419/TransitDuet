...
Generate paper result figures from local aggregated data:
  fig:training_curves    — wait time over episodes, H_hiro (TransitDuet) vs baselines
...
EVAL = LOGS / 'eval_generalization'  # mirrored by run_paper_round3.sh:agg from logs/
...
# trains for 300 episodes; the search baselines (GA, CMA-ES) train for
# 320 episodes (20 lower-only warmup + 300 search-evaluation), and the
# Fixed-headway baseline trains its lower for 300 episodes against a
# static timetable.
# ═══════════════════════════════════════════════════════════════
def fig_training_curves():
    ...
    ax.plot(eps, m, color=METHOD_COLOR['H_hiro'], linewidth=1.8,
            label='TransitDuet (HIRO, Ours)')
    ax.fill_between(eps, m - sd, m + sd, color=METHOD_COLOR['H_hiro'], alpha=0.2)

    x_max = stack.shape[1]

    # Baselines: actual training curves from history.json (Fixed: 300 eps; GA / CMA-ES: 320 eps = 20 warmup + 300 search-eval).
...
def fig_pareto_frontier():
    # Load per-seed pareto_frontier.json (one per H_hiro seed dir, generated by
    # eval_pareto_hiro.py from the validation-best checkpoint) and stack.
    all_pts = {}  # N_fleet -> list of (wait, cv, overshoot)
    for s in SEEDS:
        p_path = LOGS / f'H_hiro_seed{s}' / 'pareto_frontier.json'
        if not p_path.exists():
            print(f"  [warn] {p_path} missing; run eval_pareto_hiro.py first")
            continue
    ...
    if not all_pts:
        print("  [skip] fig_pareto_frontier: no H_hiro_seed*/pareto_frontier.json found")
        return
