...
| `eval_frequency_modules.py` | FreqDuet-only synthetic module test for causal decomposers and frequency-allocation dimensions | manual pre-RL check |
| `run_freqduet_ablation.py` | FreqDuet-only small RL ablation runner/aggregator for NoFreq, RawHistory, Harmonic, Timetable, AllFreq, and Swapped configs | manual FreqDuet check |
