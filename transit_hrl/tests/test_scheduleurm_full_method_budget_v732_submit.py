...
from scripts.submit_full_method_budget_v732_scheduleurm import (
    LINUX_CPU_NODES,
    POOL_CPU_CAPACITY,
    build_parser,
...
def args_fixture() -> argparse.Namespace:
    return argparse.Namespace(
        ...
        launch_subdir=".",
        project="Freq-HRL-v7.3.2-Budget",
        ppo_ram_mb=1024,
...
class ScheduleurmFullMethodBudgetV732SubmitTest(unittest.TestCase):
    def test_default_ladder_has_90_one_core_cells(self):
        args = normalize_args(build_parser().parse_args([
            "--run-name", "unit_budget_v732",
        ]))
        self.assertEqual(args.budgets, [64, 96])
        self.assertEqual(len(experiment_cells(args.budgets)), 90)
    ...
    def test_command_is_support_only_and_source_bound(self):
        ...
        self.assertIn("--iterations 64", command)
