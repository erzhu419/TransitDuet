...
class MujocoFrequencyAdapterTest(unittest.TestCase):
    ...
    def test_v1417_mechanism_cannot_use_the_v1416_protocol_label(self):
        ...
        with self.assertRaisesRegex(ValueError, "v17.2 mechanisms"):
            train_mujoco_method(
        ...
        with self.assertRaisesRegex(ValueError, "v17.4 mechanisms"):
            train_mujoco_method(
                lower_action_router_mode="causal_streaming_audit_projection",
                upper_action_decoder_mode="causal_smoothstep_plan",
                **common,
            )
        with self.assertRaisesRegex(ValueError, "v17.5 mechanisms"):
            train_mujoco_method(
        ...
        self.assertTrue(MUJOCO_CONTROL_PROTOCOL_VERSION_V17_4.endswith(
            "streaming_audit_projection"
        ))
        self.assertTrue(MUJOCO_CONTROL_PROTOCOL_VERSION_V17_5.endswith(
            "feasibility_normalized_projection"
        ))
    ...
    def test_router_homotopy_has_registered_linear_and_cosine_paths(self):
        ...
        self.assertEqual(
            ...
            mujoco_policy_state_dim(17, 6) + 218,
        )
        self.assertEqual(
            mujoco_policy_state_dim(
                17,
...
class MujocoControlIntegrationTest(unittest.TestCase):
    ...
    def test_v21_training_entrypoint_weights_certified_actions_by_advantage(self):
        payload, rows, model = train_mujoco_method(
            method="freq_hrl",
            env_id="HalfCheetah-v5",
            disturbance_mode="standard",
            train_seeds=[1993],
            selection_seeds=[1997],
            eval_seeds=[1999],
            steps=8,
            episode_horizon=8,
            iterations=1,
        ...
        self.assertEqual(
            payload["protocol_version"], MUJOCO_CONTROL_PROTOCOL_VERSION_V21
        )
        self.assertEqual(
            payload["projection_consistency_weighting"],
            "exp_reward_advantage",
        )
        self.assertEqual(
            payload["projection_consistency_advantage_temperature"], 1.0
        )
        self.assertEqual(
            payload["projection_consistency_advantage_weight_clip"], 5.0
        )
        self.assertEqual(
            model.config.projection_consistency_weighting,
            "exp_reward_advantage",
        )
        self.assertGreaterEqual(
            payload["history"][1][
                "lower_projection_consistency_weight_max"
            ],
            1.0,
        )
        lower_weight_summary = payload[
            "projection_consistency_weight_training"
        ]["lower"]
        self.assertEqual(
            lower_weight_summary["active_iteration_count"], 1.0
        )
        self.assertGreaterEqual(lower_weight_summary["weight_max"], 1.0)
        self.assertGreater(
            lower_weight_summary["weighted_mse_mean"], 0.0
        )
        self.assertEqual(len(rows), 1)
        self.assertEqual(
            rows[0]["terminal_reserve_certificate_violation_count"], 0.0
    ...
    def test_v23_training_uses_causal_upper_projection_target(self):
        payload, rows, model = train_mujoco_method(
            method="freq_hrl",
            env_id="HalfCheetah-v5",
            disturbance_mode="standard",
            train_seeds=[2011],
            selection_seeds=[2017],
            eval_seeds=[2027],
            steps=8,
            episode_horizon=8,
            iterations=1,
        ...
        self.assertEqual(
            payload["protocol_version"], MUJOCO_CONTROL_PROTOCOL_VERSION_V23
        )
        self.assertEqual(
            payload["upper_projection_target_aggregation"], "decision_time"
        )
        self.assertEqual(
    ...
    def test_smooth_macro_gauge_preserves_closed_loop_and_smooths_upper(self):
        observation_dim, action_dim = environment_dimensions(
            "HalfCheetah-v5", episode_horizon=128
        )
        torch.manual_seed(16021)
        np.random.seed(16021)
        model = _hierarchical_model(
            state_dim=mujoco_policy_state_dim(observation_dim, action_dim),
            action_dim=action_dim,
            hidden_dim=16,
        ...
        common = dict(
            ...
            responsibility_mode="additive",
            lower_action_router_mode="causal_smooth_macro_gauge",
            lower_action_router_alpha=0.10,
            lower_action_router_observe_strength=False,
            upper_action_decoder_mode="causal_smoothstep_plan",
            upper_constraint_mode="disabled",
            sample=False,
        ...
        self.assertLess(fixed["UpperHFPowerAbs"], control["UpperHFPowerAbs"])
    ...
    def test_streaming_audit_projection_preserves_closed_loop_with_full_state(self):
        observation_dim, action_dim = environment_dimensions(
            "HalfCheetah-v5", episode_horizon=128
        )
        router_mode = "causal_streaming_audit_projection"
        torch.manual_seed(17401)
        np.random.seed(17401)
        model = _hierarchical_model(
            state_dim=mujoco_policy_state_dim(
                observation_dim,
                action_dim,
        ...
        self.assertEqual(fixed["LowerRouterFunctionPreserving"], 1.0)
        self.assertGreater(
            fixed["LowerRouterUpperBudgetFeasibleRate"], 0.99
        )
        self.assertLessEqual(
            fixed["LowerRouterActionReconstructionRMS"], 1e-7
        )
    ...
    def test_streaming_audit_projection_trains_with_complete_fir_state(self):
        payload, rows, _ = train_mujoco_method(
            method="freq_hrl",
            env_id="HalfCheetah-v5",
            disturbance_mode="standard",
            train_seeds=(17411,),
            selection_seeds=(17413,),
            eval_seeds=(17417,),
            steps=8,
            iterations=1,
            optimizer_seed=17419,
            episode_horizon=8,
            upper_period=4,
            hidden_dim=8,
            learning_rate=3e-4,
    ...
    def test_feasibility_projection_preserves_closed_loop_and_attains_floor(self):
        observation_dim, action_dim = environment_dimensions(
            "HalfCheetah-v5", episode_horizon=128
        )
        router_mode = "causal_feasibility_normalized_audit_projection"
        torch.manual_seed(17501)
        np.random.seed(17501)
        model = _hierarchical_model(
            state_dim=mujoco_policy_state_dim(
                observation_dim,
                action_dim,
        ...
        for trace in (
            ...
            self.assertEqual(control[trace], projected[trace], trace)
        self.assertEqual(control["episode_return"], projected["episode_return"])
        self.assertLessEqual(
            projected["LowerRouterBudgetExcessRegretRMS"], 1e-7
        )
        self.assertGreaterEqual(
            projected["LowerRouterJointBudgetFeasibleRate"], 0.0
        )
        self.assertLessEqual(
            projected["LowerRouterJointBudgetFeasibleRate"], 1.0
        )
        self.assertLessEqual(
            projected["LowerRouterActionReconstructionRMS"], 1e-7
        )
        self.assertEqual(projected["protocol_valid"], 1.0)

    def test_feasibility_projection_trains_with_complete_fir_state(self):
        payload, rows, _ = train_mujoco_method(
            method="freq_hrl",
            env_id="HalfCheetah-v5",
            disturbance_mode="standard",
            train_seeds=(17511,),
            selection_seeds=(17513,),
            eval_seeds=(17517,),
            steps=8,
            iterations=1,
            optimizer_seed=17519,
            episode_horizon=8,
            upper_period=4,
            hidden_dim=8,
            learning_rate=3e-4,
