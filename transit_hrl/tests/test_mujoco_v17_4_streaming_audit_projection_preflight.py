...
from scripts import (
    mujoco_v17_3_audit_optimal_macro_gauge_preflight_spec as v17_3,
)
from scripts import (
    mujoco_v17_4_streaming_audit_projection_preflight_spec as spec,
)
from scripts.analyze_mujoco_v17_4_streaming_audit_projection_preflight import (
    analyze,
)
from scripts.run_mujoco_v17_4_paired_gauge_cell import (
    paired_intervention_audit,
    run_cell,
)
from scripts.submit_mujoco_v17_4_streaming_audit_projection_preflight_scheduleurm import (
...
def _paired_rows(
    *,
    candidate_upper_hf: float = 0.004,
    candidate_lower_lf: float = 0.0015,
):
    control = []
    candidate = []
    for mode in spec.EVALUATION_DISTURBANCE_MODES:
        for seed in spec.EVALUATION_SEEDS:
            ...
            common = {
                ...
                "LowerRouterUpperBudgetFeasibleRate": 1.0,
                "LowerRouterLowerBudgetSatisfiedRate": 1.0,
                "LowerRouterUpperBudgetViolationRMS": 0.0,
                "LowerRouterLowerBudgetViolationRMS": 0.0,
                "protocol_valid": 1.0,
                "parameter_count": 100,
                "episode_length": 512,
            ...
            control.append({
                **common,
                "UpperHFPowerAbs": 0.003,
                "LowerLFDriftAbs": 0.003,
                "LowerActionRouterStrength": spec.CONTROL_STRENGTH,
                "paired_intervention": spec.CONTROL_INTERVENTION,
            })
            candidate.append({
                **common,
                "UpperHFPowerAbs": candidate_upper_hf,
                "LowerLFDriftAbs": candidate_lower_lf,
    ...
    for environment in spec.ENVIRONMENTS:
        for optimizer_seed in spec.OPTIMIZER_SEEDS:
            control, candidate = _paired_rows(
                candidate_upper_hf=candidate_upper_hf,
                candidate_lower_lf=candidate_lower_lf,
            )
...
def test_v174_design_is_fresh_paired_and_dynamic():
    old_seeds = set(
        v17_3.OPTIMIZER_SEEDS
        + v17_3.TRAIN_SEEDS
        + v17_3.SELECTION_SEEDS
        + v17_3.EVALUATION_SEEDS
    )
    new_seeds = set(
    ...
    assert "run_mujoco_v17_4_paired_gauge_cell.py" in command
...
def test_analysis_accepts_absolute_upper_budget_with_joint_improvement(tmp_path):
    run_name = "synthetic_v174"
    ...
    assert result["eligible_for_streaming_projection_multiseed"]
    assert all(row["supported"] for row in result["cells"])
    assert all(
        row["upper_hf_relative_reduction"] < 0.0
        for row in result["cells"]
    )


def test_analysis_rejects_candidate_above_absolute_upper_budget(tmp_path):
    run_name = "synthetic_v174_upper_budget_failure"
    _write_synthetic_run(
        tmp_path,
        run_name,
        candidate_upper_hf=spec.UPPER_HF_RMS_BUDGET ** 2 + 1e-4,
        candidate_lower_lf=0.001,
    )
    result = analyze(run_name, root=tmp_path)
    assert result["status"] == spec.NOT_SUPPORTED_STATUS
    assert not result["eligible_for_streaming_projection_multiseed"]
    assert all(
        not row["gates"]["upper_hf_absolute_budget"]
        for row in result["cells"]
    )
    assert all(
        all(
            ...
            if gate != "upper_hf_absolute_budget"
        )
        for row in result["cells"]
    )
...
def test_worker_runs_one_real_paired_smoke(monkeypatch):
    ...
    assert payload["policy_filter_state_contract"] == spec.POLICY_STATE_CONTRACT
    ...
    assert rows[1]["LowerRouterUpperBudgetFeasibleRate"] > 0.99
