...
from scripts import mujoco_v14_3_partial_router_screen_spec as spec
from scripts.analyze_mujoco_v14_3_partial_router_screen import (
    _validate_router_strength,
    analyze,
)
from scripts.submit_mujoco_v14_3_partial_router_screen_scheduleurm import (
...
class MujocoV143PartialRouterScreenTest(unittest.TestCase):
    ...
    def test_development_seeds_do_not_reuse_v12_to_v142_roles(self):
        old = set()
        for module in (v12, v13, v14, v14_1, v14_2):
    ...
    def test_analyzer_requires_every_environment_condition_gate(self):
        test_arm_names = (
            ...
            "router_a004_s006_reward",
            "router_a004_s010_reward",
            "router_a010_s015_reward",
        )
        patchers = (
        ...
        arm_values = {
            ...
            "router_a004_s006_reward": (
                99.5, 0.5, 0.88, 0.94, 0.94, 0.0, 0.0025
            ),
            "router_a004_s010_reward": (
                99.5, 0.4, 0.80, 0.90, 0.90, 0.0, 0.0025
            ),
            "router_a010_s015_reward": (
                95.0, 0.2, 0.70, 0.85, 0.85, 0.0, 0.0025
            ),
        }
        with tempfile.TemporaryDirectory() as directory:
            run = Path(directory) / "results" / "unit_v14_3"
            merged = run / "merged"
        ...
        self.assertEqual(
            decision["selected_arm"], "router_a004_s010_reward"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            decision["arm_status"]["router_a004_s010_reward"][
                "passed_gate_count"
            ],
            4,
        )
        self.assertTrue(all(
            ...
            if row["arm"] == "router_a004_s010_reward"
        ))
        self.assertTrue(all(
            not row["reward_noninferiority_pass"]
            for row in decision["environment_condition_rows"]
            if row["arm"] == "router_a010_s015_reward"
        ))
        at_floor_rows = [
