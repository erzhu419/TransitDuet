...
from scripts import pointmaze_plan_validity_stage8b_spec as spec
from scripts import pointmaze_plan_value_stage8_spec as stage8
from scripts.submit_hyperparameter_pilot_scheduleurm import LINUX_CPU_NODES
from scripts.submit_pointmaze_plan_validity_stage8b_scheduleurm import (
    task_specification,
    training_command,
)


class PointMazePlanValidityStageEightBSchedulerTest(unittest.TestCase):
    def test_matrix_and_four_seed_roles_are_frozen_and_fresh(self):
        self.assertEqual(len(spec.OPTIMIZER_SEEDS), 8)
        self.assertEqual(len(spec.cells(preflight=False)), 8)
        self.assertEqual(len(spec.cells(preflight=True)), 1)
        self.assertEqual(spec.PREFLIGHT_OPTIMIZER_SEEDS, (206001,))
        old = {
            seed
            for root in (
                *stage8.PREFLIGHT_OPTIMIZER_SEEDS,
                *stage8.OPTIMIZER_SEEDS,
            )
            for values in stage8.seed_roles(root).values()
            for seed in values
        }
        observed = set()
    ...
    def test_preflight_is_small_and_preserves_all_four_roles(self):
        ...
        for role in ("train", "selection", "branch_fit", "branch_eval"):
            self.assertEqual(len(options[role]), 1)
    ...
    def test_task_is_dynamic_single_core_and_syncs_compact_result(self):
        cell = spec.cells(preflight=True)[0]
        command = training_command("unit_stage8b", cell, preflight=True)
        self.assertIn(spec.RUNNER_SCRIPT, command)
        self.assertIn("--branch-window-seconds 0.5", command)
        self.assertIn("--max-events-per-class 1", command)
        self.assertIn("--branch-fit-seeds", command)
        self.assertIn("--branch-eval-seeds", command)
        task = task_specification("unit_stage8b", cell, preflight=True)
    ...
    def test_gate_is_conjunctive_and_forbids_root_extension(self):
        self.assertEqual(
            ...
            "renew_ise_advantage_keep_minus_renew",
        )
        self.assertEqual(
            spec.CLAIM_GATE["authorization_scope"],
            "stage9_trigger_development_only",
        )
        self.assertEqual(
