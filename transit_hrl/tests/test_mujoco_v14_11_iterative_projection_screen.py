...
from scripts import mujoco_v14_10_deployment_aligned_screen_spec as v14_10
from scripts import mujoco_v14_11_iterative_projection_screen_spec as spec
from freq_hrl.experiments.mujoco.control_validation import (
    MUJOCO_CONTROL_PROTOCOL_VERSION,
)
from scripts.analyze_mujoco_v14_11_iterative_projection_screen import analyze
from scripts.analyze_mujoco_v14_11_iterative_projection_preflight import (
    analyze_preflight,
)
from scripts.submit_mujoco_v14_11_iterative_projection_screen_scheduleurm import (
    ANCHOR_ARM,
...
class MujocoV1411IterativeProjectionScreenTest(unittest.TestCase):
    @staticmethod
    def _args() -> argparse.Namespace:
        return argparse.Namespace(
            run_name="unit_v14_11_screen",
    ...
    def test_registry_commands_and_dynamic_scheduler_contract(self):
        ...
        self.assertEqual(
            sum(cell[0] == "continuation" for cell in cells), 432
        )
        self.assertEqual(len(selected_experiment_cells(self._args())), 10)
        ...
        learned_arm = "relative_s050_asym_u003_l008_s13_r05_k8"
        ...
        self.assertIn(
            "--upper-deployment-frequency-dual-lr 0.03", learned
        )
        self.assertIn(
            "--lower-deployment-frequency-dual-lr 0.08", learned
        )
        self.assertIn(
            "--upper-deployment-frequency-step-scale 1.0", learned
        )
        self.assertIn(
            "--upper-deployment-frequency-max-projection-steps 8", learned
        )
        self.assertIn(
            "--lower-deployment-frequency-reward-tolerance 0.0001", learned
        )
        self.assertIn(
    ...
    def test_all_v1411_seed_roles_are_fresh_and_disjoint(self):
        old = set()
        for module in (
            v12, v13, v14, v14_1, v14_2, v14_3, v14_4, v14_5, v14_6,
            v14_7, v14_8, v14_9, v14_10,
        ):
    ...
    def test_analyzer_uses_separate_calibration_and_matched_controls(self):
        legacy = spec.LEGACY_ONE_STEP_ARM
        learned = "relative_s050_asym_u003_l008_s13_r05_k8"
        arm_names = (
        ...
        patchers = (
            ...
            patch.object(spec, "LEARNED_ARMS", (legacy, learned)),
            patch.object(spec, "ITERATIVE_ARMS", (learned,)),
            patch.object(spec, "CANDIDATE_ARMS", (legacy, learned)),
            patch.object(
                spec,
                "EVALUATED_ARMS",
                (spec.CALIBRATION_ARM, legacy, learned),
            ),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            values = {
                ...
                legacy: (
                    101.0, 0.70, 0.60, 0.80, 0.0028, 0.0035, "l", 0.025, "l",
                ),
                learned: (
            ...
            for arm, arm_values in values.items():
                for seed in spec.OPTIMIZER_SEEDS:
                    ...
                    active = arm in (legacy, learned)
                    projection_steps = int(spec.ARMS[arm][
                        "upper_deployment_frequency_max_projection_steps"
                    ]) if active else 1
                    accepted_steps = 2 if arm == learned else int(active)
                    power_after = (
                        0.9 if arm == learned else 0.99 if arm == legacy else 1.0
                    )
                    (cell / "training_history.json").write_text(json.dumps([{
                        "upper_deployment_frequency_enabled": float(active),
                        "upper_deployment_frequency_guard_attempted": float(active),
                        "upper_deployment_frequency_guard_accepted": float(
                            accepted_steps
                        ),
                        "upper_deployment_frequency_projection_steps_requested": (
                            projection_steps
                        ),
                        "upper_deployment_frequency_projection_steps_attempted": (
                            accepted_steps
                        ),
                        "upper_deployment_frequency_projection_steps_accepted": (
                            accepted_steps
                        ),
                        "upper_deployment_frequency_projection_reward_tolerance": (
                            spec.ARMS[arm][
                                "upper_deployment_frequency_reward_tolerance"
                            ]
                        ),
                        "upper_deployment_frequency_projection_target_reached_before": 0.0,
                        "upper_deployment_frequency_projection_target_reached_after": 1.0,
                        "upper_deployment_frequency_guard_reward_loss_delta": 0.0,
                        "upper_deployment_frequency_power_before": 1.0,
                        "upper_deployment_frequency_power_after": power_after,
                        "lower_deployment_frequency_enabled": float(active),
                        "lower_deployment_frequency_guard_attempted": float(active),
                        "lower_deployment_frequency_guard_accepted": float(
                            accepted_steps
                        ),
                        "lower_deployment_frequency_projection_steps_requested": (
                            projection_steps
                        ),
                        "lower_deployment_frequency_projection_steps_attempted": (
                            accepted_steps
                        ),
                        "lower_deployment_frequency_projection_steps_accepted": (
                            accepted_steps
                        ),
                        "lower_deployment_frequency_projection_reward_tolerance": (
                            spec.ARMS[arm][
                                "lower_deployment_frequency_reward_tolerance"
                            ]
                        ),
                        "lower_deployment_frequency_projection_target_reached_before": 0.0,
                        "lower_deployment_frequency_projection_target_reached_after": 1.0,
                        "lower_deployment_frequency_guard_reward_loss_delta": 0.0,
                        "lower_deployment_frequency_power_before": 1.0,
                        "lower_deployment_frequency_power_after": power_after,
                    }]), encoding="utf-8")
    ...
    def test_scoped_preflight_requires_real_projection_and_can_expand(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            values = {
                ...
                legacy: (
                    101.0, 0.70, 0.60, 0.80, 0.0028, 0.0035, "l", 0.025,
                ),
                learned: (
