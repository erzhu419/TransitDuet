"""Merge and gate the source-bound Freq-HRL v7.3.2 budget ladder."""
...
from . import full_method_budget_plan_v732 as plan
...
def _row_passes(row: dict[str, Any]) -> bool:
    return (
        ...
        and float(row["checkpoint_boundary_replicate_fraction"])
        <= plan.MAX_BOUNDARY_REPLICATE_FRACTION
    )
...
def summarize_budget_cells(
    ...
    *,
    ...
    if not source_verified:
        ...
        status = "blocked_budget_limited_at_128"
...
def write_budget_decision(output_dir: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Freq-HRL v7.3.2 Training-Budget Decision",
        "",
